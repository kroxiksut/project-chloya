...
- [1.2. Почему полная автономность не является основной целью](#12-почему-полная-автономность-не-является-основной-целью)
  - [1.2.1. Что уже удавалось создавать в автономном режиме](#121-что-уже-удавалось-создавать-в-автономном-режиме)
  - [1.2.2. Почему отдельные демонстрации не доказывают общую надёжность](#122-почему-отдельные-демонстрации-не-доказывают-общую-надёжность)
  - [1.2.3. Экономическая цена автономности без цели и критериев остановки](#123-экономическая-цена-автономности-без-цели-и-критериев-остановки)
  - [1.2.4. Позиция CHLOYA](#124-позиция-chloya)
...
При этом компилятор имеет существенные ограничения. Для отдельных этапов загрузки x86 он обращается к GCC, не содержит полностью собственного стабильного ассемблера и компоновщика и не является полноценной заменой промышленного компилятора общего назначения [41](https://www.anthropic.com/engineering/building-c-compiler). Следовательно, факт сборки ядра Linux подтверждает высокий уровень технической результативности, но не равен [доказательству][g-evidence] производственной готовности всего решения.
...
При этом испытания METR в основном состоят из самодостаточных и хорошо сформулированных задач с ясными критериями и возможностью автоматической проверки. METR отдельно предупреждает, что такие результаты нельзя напрямую приравнивать к повседневной работе специалиста, обладающего долговременным знанием продукта, пользователей и организации. Измерения для задач продолжительностью более 16 человеко-часов организация пока считает недостаточно надёжными [46](https://metr.org/time-horizons/).

Рисунок 1.3 — Временной горизонт выполнения задач агентами [46](https://metr.org/time-horizons/).

![Временной горизонт выполнения задач агентами](../../assets/why-new-methodology/metr-task-time-horizon.png)

Рассмотренные примеры позволяют сделать несколько выводов.

Во-первых, длительная автономная работа уже применима к крупным, но ограниченным и хорошо проверяемым задачам. Особенно успешны случаи, где существует сильный проверочный механизм: компилятор либо собирает программу и проходит тесты, либо нет; игра либо запускается и реализует заданные механики, либо нет.

Во-вторых, результат определяется не только качеством модели. Существенную роль играют подробность спецификации, структура проекта, доступные инструменты, организация параллельной работы, тесты, возможность запуска результата, бюджет, ограничения среды и правила восстановления после ошибок.

В-третьих, автоматическая проверка не всегда отвечает на главный вопрос. Тесты могут доказать, что компилятор собирает ядро или что персонаж движется по трассе, но не доказывают, что продукт нужен пользователям, архитектура подходит для дальнейшего развития, интерфейс удобен, а реализация соответствует неформальному замыслу владельца.

В-четвёртых, большой объём созданного кода, количество токенов или продолжительность непрерывной работы не должны сами по себе считаться показателями эффективности.
...
Тарифы приведены за один миллион [токенов модели][g-model-token] [47](https://developers.openai.com/api/docs/pricing) по состоянию на 19.07.2026.
...
При том же условном соотношении 80/20 расход 100 миллионов [токенов модели][g-model-token] составил бы приблизительно 200, 500 или 1000 долларов для трёх перечисленных моделей. Один миллиард [токенов модели][g-model-token] стоил бы соответственно около 2, 5 или 10 тысяч долларов [47](https://developers.openai.com/api/docs/pricing).
...
Эксперимент с компилятором нельзя считать бесцельной тратой ресурсов. Он имел исследовательскую цель, проверяемые критерии и позволил изучить пределы [многоагентной работы][g-multi-agent-work]. Однако он показывает верхнюю границу того, во что может превратиться длительная автономная работа. Если сопоставимый объём вычислений расходуется на продукт без подтверждённой потребности, критериев приёмки и архитектурных ограничений, значительная часть стоимости может быть потеряна.
...
Отдельный источник расходов — многократная передача агентам одного и того же проектного [контекста][g-context]. Он зависит уже не только от режима автономности, но и от архитектуры модулей, [контрактов][g-contract] и [проектной памяти][g-project-memory], поэтому рассматривается в разделе 1.3.
...
- бюджет определяет допустимую стоимость выполнения задачи, но не заменяет управление [контекстом][g-context]: даже большой согласованный бюджет не оправдывает передачу исполнителю нерелевантных или многократно дублируемых материалов;
...
- постановке цели;
- определении границ и [допустимого риска][g-acceptable-risk];
- утверждении архитектурно значимых решений;
- оценке того, создаёт ли результат реальную ценность;
- принятии [остаточного риска][g-residual-risk];
...
Таким образом, целевой показатель CHLOYA — не максимальное число часов, в течение которых агент способен работать без человека, и не минимальное количество человеческих действий. Цель состоит в том, чтобы предоставить агенту **максимально полезную автономность внутри явно заданных границ**, не теряя соответствия человеческому замыслу, проверяемости, [обратимости][g-reversibility], экономической обоснованности и ответственности.
...
Если любой исполнитель перед каждым изменением вынужден заново изучать весь репозиторий, историю решений, документацию и результаты работы других агентов, стоимость разработки растёт вместе с размером проекта. Одни и те же сведения многократно передаются моделям, занимают [контекстное окно][g-context-window], увеличивают задержку и расширяют область результата, которую затем должен проверить человек.

Поэтому в CHLOYA [контекст][g-context] рассматривается не как бесплатное приложение к задаче, а как ограниченный архитектурный ресурс наряду с вычислительным бюджетом, временем исполнения и [полномочиями][g-authority] агента.

### 1.3.1. Контекстное окно не является памятью проекта

Проект может содержать код, документацию, журнал решений, требования, результаты испытаний, историю изменений и накопленные знания о предметной области. Однако наличие этих материалов в репозитории или хранилище ещё не означает, что все они должны одновременно находиться в [активном контексте][g-active-context] конкретного агента.

Необходимо различать:

- **[память проекта][g-project-memory]** — всё сохранённое знание, которое может потребоваться в будущем;
- **[контекст по запросу][g-context-on-demand]** — материалы, которые агент при необходимости может запросить;
- **[активный контекст][g-active-context]** — сведения, непосредственно переданные модели для выполнения текущего шага;
- **[минимально достаточный контекст][g-minimal-context]** — минимальный набор информации, без которого задача не может быть правильно понята и выполнена.

Даже если [контекстное окно][g-context-window] модели технически позволяет вместить значительную часть репозитория, это не гарантирует, что модель одинаково надёжно найдёт и применит каждое важное ограничение.

Исследование *Lost in the Middle* показало, что качество использования длинного [контекста][g-context] может зависеть от положения значимой информации. В проведённых авторами экспериментах модели лучше использовали сведения, расположенные ближе к началу или концу [контекста][g-context], и хуже — сведения из его середины [48](https://arxiv.org/abs/2307.03172). Способность принять длинный вход поэтому не равна способности одинаково надёжно использовать всё его содержание.

Рисунок 1.4 — Влияние положения значимой информации в длинном контексте на точность ответа модели [48](https://arxiv.org/abs/2307.03172).
...
Anthropic определяет [проектирование контекста][g-context-engineering] как отбор и поддержание оптимального набора [токенов модели][g-model-token], доступных во время выполнения задачи. [Контекст][g-context] при этом является конечным ресурсом: каждый дополнительный фрагмент может помогать, не влиять на результат либо отвлекать модель от более значимых сведений. Для агентных систем Anthropic отдельно рекомендует сочетать предварительно передаваемую информацию с загрузкой дополнительных данных по мере необходимости [49](https://www.anthropic.com/engineering/effective-context-engineering-for-ai-agents).

Рисунок 1.5 — Архитектура многоагентной исследовательской системы как пример проектирования контекста [49](https://www.anthropic.com/engineering/effective-context-engineering-for-ai-agents).
...
> **[Память проекта][g-project-memory] должна быть достаточно полной и восстанавливаемой, а [активный контекст][g-active-context] агента — актуальным, доверенным и минимально достаточным для его задачи.**

«Минимально достаточный» не означает «максимально короткий». Искусственное сокращение [контекста][g-context] так же опасно, как его бесконтрольное расширение.
...
**Финансовая стоимость.** Повторяющиеся инструкции, документация, исходный код и результаты инструментов увеличивают объём входных [токенов модели][g-model-token]. В [многоагентной работе][g-multi-agent-work] один и тот же материал может передаваться нескольким исполнителям и повторяться в нескольких итерациях.

**Задержка.** Большой [контекст][g-context] необходимо собрать, передать и обработать. Это увеличивает время между постановкой задачи и получением результата.
...
**Рост [доверительной поверхности][g-trust-surface].** Каждый добавленный документ или результат инструмента может содержать ошибочные сведения, устаревшие инструкции, конфиденциальные данные либо недоверенное содержимое.
...
- нарушить [внешний контракт][g-external-contract];
...
- документированные [контракты][g-contract];
...
Если эти сведения формализованы, исполнителю не требуется читать внутреннюю реализацию всех соседних компонентов. Он может работать на основании [контракта][g-contract] и запрашивать дополнительный [контекст][g-context] только при обнаружении неопределённости.
...
- с какими [контрактами][g-contract] взаимодействует;
- какие ограничения являются обязательными;
- какие соседние компоненты могут быть затронуты;
- в каких случаях он обязан остановиться и запросить расширение [контекста][g-context].
...
Такое представление называется в CHLOYA **[семантической передачей изменения][g-semantic-handoff]**.
...
Семантическая передача не отменяет доступ к исходной реализации. Если агент обнаруживает противоречие, недостаточность [контракта][g-contract] или непредусмотренную зависимость, он должен иметь возможность запросить исходные материалы. Но полное чтение чужого модуля становится исключением, обоснованным конкретной необходимостью, а не обязательным началом каждой задачи.



### 1.3.5. Размножение контекста в многоагентной работе

Стоимость [контекста][g-context] особенно быстро растёт при параллельной работе агентов.

Рассмотрим условный проект, в котором значимая часть репозитория, инструкций и истории занимает 400 тысяч [токенов модели][g-model-token]. Над изменением работают пять агентов, каждый выполняет четыре существенные итерации.

Если каждый раз передавать каждому агенту весь объём, суммарная обработка составит:

> 400 000 × 5 × 4 = 8 000 000 входных токенов модели.

Если для каждого исполнителя формируется локальная контекстная капсула объёмом 40 тысяч токенов модели:

> 40 000 × 5 × 4 = 800 000 входных токенов модели.
...
Опыт Anthropic при создании [многоагентной работы][g-multi-agent-work] показывает, что слабая постановка подзадач приводила к дублированию работы, пропущенным областям и чрезмерному количеству исполнителей. Для снижения таких потерь координатору потребовались правила масштаба, чёткие цели подзадач, формат результата и явные границы работы каждого агента [52](https://www.anthropic.com/engineering/multi-agent-research-system).
...
- существует [условие остановки][g-stopping-condition] дальнейшего размножения задач.
...
Однако кеширование решает проблему повторной обработки, а не проблему релевантности. Дешёвый нерелевантный [контекст][g-context] остаётся нерелевантным. Кроме того, изменение ранней части запроса может уменьшить долю повторно используемого кеша.

### 1.3.6. Контекстные слои и капсула задачи

В CHLOYA целесообразно различать три уровня проектного [контекста][g-context].

| Уровень | Содержание | Режим использования |
|---|---|---|
| [Активный контекст][g-active-context] | Текущая задача, локальный код, действующие [контракты][g-contract], ограничения, риски и критерии готовности | Передаётся исполнителю непосредственно |
| [Контекст по запросу][g-context-on-demand] | Соседние модули, расширенная архитектурная документация, связанные решения и дополнительные тесты | Загружается при обнаружении зависимости или неопределённости |
| Долговременная [память проекта][g-project-memory] | Полная история решений, завершённые задачи, журналы, [доказательства][g-evidence], старые версии и большие отчёты | Хранится вне активного окна и используется выборочно |

Базовая [контекстная капсула задачи][g-task-context-capsule] должна содержать:

- цель;
- ожидаемый артефакт;
- область разрешённых изменений;
- код, значимый для конкретной задачи;
- применимые [контракты][g-contract];
- актуальные архитектурные решения;
- запрещённые действия;
- критерии приёмки;
- бюджет времени и вычислений;
- уровень доверия к источникам;
- признаки, требующие расширения [контекста][g-context];
- формат возвращаемого результата.

Состав капсулы зависит от риска. Для [локального изменения][g-local-change] документации может быть достаточно нескольких файлов и правил оформления. Для изменения авторизации, схемы данных, платёжной логики или механизма обновления потребуется более широкий системный [контекст][g-context] и дополнительные проверки.
...
4. запросить конкретное расширение [контекста][g-context];
5. оценить влияние расширения на стоимость и границы задачи;
6. продолжить работу после получения разрешения или уточнённого [контракта][g-contract].

Такой механизм предотвращает обе крайности: бесконтрольное чтение всей системы и опасную работу в искусственно узком [контексте][g-context].
...
Поэтому [контекст][g-context] проектируется вместе с модулями, [контрактами][g-contract] и [полномочиями][g-authority].
...
- доказуемую связь между полученным [контекстом][g-context] и выполняемым изменением.
...
> **[Проектная память][g-project-memory] должна быть полной и восстанавливаемой. Рабочий [контекст][g-context] должен быть локальным, актуальным, доверенным, минимально достаточным и расширяемым по явной необходимости.**

## 1.4. Конфиденциальность, владение проектными знаниями и независимость от поставщика

Код, архитектура, производственные журналы, данные клиентов, коммерческие алгоритмы, внутренняя переписка и сведения об эксплуатации могут содержать конфиденциальную информацию. Поэтому техническая доступность материала для агента не должна автоматически означать разрешение передать его [внешней модели][g-external-model].
...
Особое место занимают пароли, ключи и токены доступа. Они не должны включаться в [контекст][g-context] модели как обычный текст. Когда агенту необходимо совершить разрешённое действие, безопаснее предоставить ему ограниченную операцию через отдельный инструмент или секрет-брокер, не раскрывая само значение секрета.
...
Критерии качества также не исчерпываются автоматическими тестами. Владелец проекта может учитывать понятность реализации, удобство сопровождения, соответствие исходному замыслу, [допустимый риск][g-acceptable-risk], стоимость эксплуатации и влияние на пользователя. Эти критерии необходимо сохранять как часть [проектной памяти][g-project-memory].
...
Корпоративный аккаунт обычно предоставляет дополнительные средства управления, аудита и договорной защиты. Однако сам факт использования бизнес-тарифа не превращает любые данные компании или заказчика в допустимый [контекст][g-context].

[API][g-api] позволяет точнее определять состав передаваемой информации и встраивать собственную предварительную фильтрацию. Вместе с тем необходимо учитывать не только основной запрос, но и серверное состояние, журналы мониторинга, хранение файлов, кеширование и работу подключённых инструментов.

Облачный программирующий агент обладает более широкой [доверительной поверхностью][g-trust-surface], чем обычный диалог. Он может читать репозиторий, выполнять команды, обращаться к системе задач, использовать браузер и взаимодействовать с внешними сервисами. Поэтому его следует оценивать как полноценного участника инфраструктуры, а не только как языковую модель.

Управляемая корпоративная платформа может обеспечивать централизованные [политики][g-policy], региональное размещение, интеграцию с системой идентификации и аудит. Фактические гарантии всё равно зависят от договора, настроек и набора используемых функций.
...
Полностью [локальная модель][g-local-model] без внешней сети подходит для наиболее чувствительных материалов, хотя её качество, скорость и доступные инструменты могут уступать облачным решениям.

Наконец, любой из перечисленных вариантов может использовать браузер, поиск, плагины или [MCP][g-mcp]-серверы. Даже если основная модель находится в допустимой среде, внешний инструмент способен сохранить данные, передать их другой стороне или вернуть значительно больше информации, чем требовалось [5](https://modelcontextprotocol.io/specification/2025-11-25/server/tools), [27](https://modelcontextprotocol.io/docs/tutorials/security/security_best_practices).
...
OpenAI указывает, что данные [API][g-api] по умолчанию не используются для обучения моделей. При стандартной обработке пользовательское содержимое может сохраняться в журналах мониторинга злоупотреблений до 30 дней; для соответствующих требованиям организаций предусмотрены дополнительные режимы Modified Abuse Monitoring и Zero Data Retention [54](https://developers.openai.com/api/docs/guides/your-data).

Anthropic также отделяет стандартные условия хранения [API][g-api] от соглашения Zero Data Retention. Такой режим применяется на уровне организации, но отдельные возможности могут иметь собственные ограничения или не поддерживать нулевое хранение [55](https://docs.anthropic.com/en/docs/build-with-claude/zero-data-retention).
...
Развитие генеративного ИИ привело к появлению отдельного [посреднического слоя][g-intermediary-layer] инфраструктуры, расположенного между пользователем и конкретной моделью. Через единый интерфейс или [API][g-api] такие системы предоставляют доступ сразу к нескольким поставщикам, выбирают модель по стоимости и качеству, переключают запросы при сбоях, ведут общую историю и иногда сохраняют память между диалогами.

Под одним понятием «агрегатор» при этом скрываются существенно различающиеся решения. Потребительская платформа объединяет множество моделей и пользовательских приложений. Маршрутизатор [API][g-api] позволяет программно выбирать поставщика. Программирующий помощник получает доступ к коду и рабочей среде. Облачная корпоративная платформа предоставляет организации управляемый каталог моделей. Наконец, сама компания может развернуть внутренний шлюз поверх [локальных моделей][g-local-model] и внешних систем.

Объединяет их наличие промежуточного слоя. Однако его значение зависит не от названия и не от страны происхождения, а от архитектуры: кто управляет маршрутом, какие участники получают содержимое, где сохраняется история и какие последствия может иметь раскрытие [контекста][g-context].

#### Польза внешних многомодельных платформ

Внешние агрегаторы решают реальную практическую задачу. Вместо отдельных договоров, ключей, библиотек и способов оплаты проект получает одну точку подключения. Модели можно сравнивать по качеству, стоимости и времени ответа, а при недоступности основного поставщика использовать резервный маршрут. OpenRouter, например, позволяет ограничивать список поставщиков, задавать порядок маршрутизации и направлять запросы только конечным точкам с [политикой][g-policy] нулевого хранения [57](https://openrouter.ai/docs/guides/routing/provider-selection), [58](https://openrouter.ai/docs/guides/privacy/data-collection).
...
Проблема возникает тогда, когда единый интерфейс воспринимается как прозрачный переключатель, который не участвует в обработке. Для выбора модели и возврата ответа посредник технически получает переданный [контекст][g-context]. В зависимости от устройства сервиса он может сохранять историю, вести журналы, формировать пользовательскую память, выполнять аналитику и направлять части одного рабочего процесса разным поставщикам.
...
Если внутри платформы работают сторонние боты, приложения или плагины, к этой цепочке добавляются их разработчики и используемые ими сервисы. Общая память создаёт ещё один канал: сведения, сообщённые одной модели, могут позднее войти в [контекст][g-context] другой.
...
Это не означает, что платформа нарушает обещания. Напротив, открытое описание цепочки позволяет оценить её заранее. Оно показывает, что удобное переключение между моделями расширяет круг систем, которым потенциально доступен [контекст][g-context].
...
Удаление имени, адреса или идентификатора аккаунта также не устраняет ценность программы. Формально обезличенный фрагмент способен раскрыть коммерческий алгоритм, структуру [API][g-api], правила авторизации, внутренние сущности, неопубликованную функцию или неисправленную уязвимость.

Поэтому обезличивание персональных данных и обезвреживание проектного [контекста][g-context] являются разными задачами. Код может не содержать никаких сведений о физическом лице, но оставаться коммерческой тайной, объектом авторских прав или материалом, передача которого ограничена договором.
...
Проверка посредника не заканчивается в момент первоначального подключения. Пользовательские соглашения, [политика][g-policy] хранения, набор поставщиков и назначение отдельных функций способны меняться уже после внедрения.

OpenRouter прямо предусматривает возможность изменить [политику][g-policy] конфиденциальности «at any time, without prior notice». Для существенных изменений зарегистрированным пользователям обещано предварительное уведомление, однако продолжение использования после публикации новой редакции означает её принятие [59](https://openrouter.ai/terms).

Это не уникальное свойство конкретной платформы, а обычный риск облачного сервиса. Изменения могут быть вынесены в отдельную [политику][g-policy] модели, описание новой памяти, правила бесплатной программы или настройки журналирования. В результате пользователь замечает новую функцию, но не всегда сразу понимает, что изменился круг получателей данных или предоставленная лицензия.
...
Чем тяжелее последствия раскрытия, тем меньше [контекста][g-context] следует передавать внешнему посреднику даже при благоприятной редакции его условий.
...
> **[История взаимодействия][g-interaction-history] с ИИ может иметь самостоятельную ценность и поэтому нуждается в защите не только от случайной утечки, но и от целенаправленного массового сбора.**
...
Организация может пойти дальше и развернуть собственный шлюз. LiteLLM, например, предоставляет единый [API][g-api] к множеству моделей, централизованную авторизацию, ограничения бюджета, журналирование и резервную маршрутизацию [66](https://docs.litellm.ai/). Open WebUI позволяет подключать через самостоятельно размещённый интерфейс как [локальные модели][g-local-model], так и внешние [API][g-api]; при полностью изолированном развёртывании внешние функции отключаются, а [локальные модели][g-local-model] продолжают работать внутри сети [67](https://docs.openwebui.com/getting-started/quick-start/connect-a-provider/).

Такой шлюз способен выполнять несколько принципиально разных ролей.

Если модели, поиск, журналы и память расположены внутри контролируемого контура, организация получает единое рабочее пространство без передачи [контекста][g-context] публичному агрегатору. Однако внутренняя платформа концентрирует значительную часть корпоративных знаний и сама становится критически важной системой.

Если тот же интерфейс обращается к внешним [API][g-api], он уменьшает хаотичное использование сотрудниками случайных чатов и позволяет централизованно выбирать поставщиков, фильтровать секреты, учитывать расходы и отключать нежелательные маршруты. При этом запрос всё равно покидает инфраструктуру организации и подчиняется условиям конечной модели.

Наиболее гибок смешанный шлюз. Публичный текст можно направить недорогой [внешней модели][g-external-model]. Внутренний, но некритичный документ — корпоративному [API][g-api] с договорными гарантиями. Закрытый код или чувствительный журнал — [локальной модели][g-local-model]. Секреты не попадают в текстовый [контекст][g-context], а необходимые действия выполняются через ограниченные инструменты.

Такой посредник реализует не только выбор модели, но и [политику][g-policy] проекта:

> содержание задачи и класс данных → допустимые исполнители → выбор по качеству, стоимости и доступности → выполнение → сохранение результата в нейтральной [проектной памяти][g-project-memory].
...
CHLOYA не вводит универсального запрета на посредников. Оценка зависит от ценности [контекста][g-context] и возможного ущерба, а не от самого слова «агрегатор».
...
Непубличные документы и рабочая диагностика требуют более строгого режима. Здесь уже имеют значение выбранные поставщики, запрет обучения, [политика][g-policy] хранения, контролируемая память и возможность удалить или экспортировать историю.
...
Для такого [контекста][g-context] может потребоваться корпоративный канал с договорными гарантиями, локальная обработка, изолированный внутренний шлюз либо отказ от использования модели на соответствующем этапе.
...
> **Что произойдёт с человеком, заказчиком и проектом, если переданный [контекст][g-context] будет сохранён, использован повторно, получен другим поставщиком или станет публичным?**

Если последствия заранее признаны приемлемыми, посредник может быть экономически и технически оправдан. Если возможный ущерб неприемлем, благоприятной оферты и настройки конфиденциальности недостаточно: [контекст][g-context] необходимо сократить, обезвредить, направить в другую среду или не передавать вовсе.
...
Задача CHLOYA состоит в другом: сохранить цели, правила, [контракты][g-contract], решения и накопленные знания в нейтральном виде, чтобы для каждой работы можно было выбрать наиболее подходящий способ выполнения.

В одном случае таким способом станет прямой [API][g-api]. В другом — внешний агрегатор с привлекательной стоимостью. Организация может использовать управляемую облачную платформу, собственный смешанный шлюз или полностью [локальную модель][g-local-model]. Простую операцию иногда надёжнее выполнить специализированным алгоритмом без языковой модели, а наиболее чувствительное решение оставить человеку.

CHLOYA не определяет единственного правильного поставщика и не подменяет выбор пользователя собственным сервисом. Она создаёт условия, при которых инструменты можно сравнивать и заменять по цели, качеству, стоимости, конфиденциальности и [допустимому риску][g-acceptable-risk] без утраты проектного состояния.

Итоговый принцип подраздела:

> **[Посреднический слой][g-intermediary-layer] не является защитой или угрозой сам по себе. Внешний агрегатор способен расширить доверительную цепочку, а правильно устроенный корпоративный шлюз — ограничить и контролировать её. Выбор оправдан тогда, когда маршрут соответствует цели задачи, ценности [контекста][g-context] и допустимым последствиям раскрытия. CHLOYA не занимает место посредника, а помогает проекту осознанно выбирать и заменять подходящие инструменты.**
...
Внутренняя [политика][g-policy] организации вправе быть строже минимальных требований закона или договора. Кроме того, даже разрешённые данные не следует передавать полностью, если задачу можно решить по сокращённой или обезличенной выборке.

До формирования внешнего [контекста][g-context] необходимо ответить на несколько вопросов.
...
После этого выбирается допустимая среда. Одни сведения можно передать внешнему [API][g-api], другие требуют локальной обработки, изолированного инструмента или участия человека.
...
Также фиксируется круг дополнительных компонентов, которые получат доступ: поисковые службы, браузер, терминал, вспомогательные агенты, системы наблюдаемости и внешние [API][g-api].

Для чувствительных материалов сохраняется запись о составе переданного [контекста][g-context], цели, получателе и основании обработки.

Некоторые данные нельзя отправлять модели как обычный [контекст][g-context] независимо от удобства такого решения.
...
> **[Политика][g-policy] поставщика описывает свойства среды, но решение о допустимости передачи остаётся ответственностью владельца проекта и применимой [политики][g-policy] данных.**
...
На входе устанавливаются происхождение информации и [полномочия][g-authority] проекта. Данные самой организации, заказчика и пользователя могут подчиняться разным правилам.

При сборе [контекста][g-context] выбираются только относящиеся к задаче материалы. Возможность прочитать источник не означает, что его содержимое целиком должно оказаться в запросе.
...
Инструменты получают отдельные [полномочия][g-authority]. Разрешение анализировать информацию не означает автоматического права передавать её браузеру, терминалу, базе данных или [MCP][g-mcp]-серверу.

Вспомогательный агент работает со своей ограниченной капсулой и не наследует весь [контекст][g-context] координатора без явной необходимости.
...
Для чувствительного [контекста][g-context] применяется сочетание мер, а не один универсальный фильтр.
...
> **Маршрутизация и сокращение [контекста][g-context] должны происходить до вызова модели, а не после обнаружения утечки.**


Рисунок 1.6 — Маршрутизация проектного контекста по доверительным средам.
...
Задача сохраняет цель, область, ограничения и критерии приёмки. Архитектурное решение — выбранный вариант, причины и отклонённые альтернативы. Контракт — проверяемые условия взаимодействия. Риск — возможный ущерб, меры контроля и остаточную неопределённость. [Доказательство][g-evidence] связывает утверждение о готовности с тестом или отчётом. Пакет изменения описывает состав модификации, её последствия и порядок активации. [Handoff][g-handoff] позволяет продолжить работу без обязательного доступа к предыдущей переписке.
...
Полностью устранить различия между агентными средами невозможно. Они используют собственные файлы инструкций, форматы навыков, команды, [API][g-api], схемы разрешений и механизмы памяти.

CHLOYA не запрещает такие возможности: они могут заметно улучшать работу. Однако зависимость от них должна оставаться локальной и не проникать в каноническую [память проекта][g-project-memory].

Для этого выделяются два слоя.

#### Переносимое ядро

В нейтральном формате хранятся цель проекта, карта модулей, [контракты][g-contract], задачи, решения, риски, [доказательства][g-evidence], пакеты изменений, правила данных, правила выбора моделей и история утверждённых переходов.

Цель объясняет, какую проблему решает проект. Карта модулей задаёт границы ответственности. [Контракты][g-contract] определяют условия взаимодействия. Решения сохраняют не только итог, но и основания выбора. Риски показывают возможные последствия ошибки. [Доказательства][g-evidence] подтверждают готовность воспроизводимыми проверками. Правила данных устанавливают допустимые среды обработки.

В совокупности эти материалы образуют каноническое состояние, достаточное для восстановления и продолжения работы.

#### Адаптер поставщика

[Адаптер исполнителя][g-executor-adapter] переводит нейтральное состояние в форму конкретного инструмента.

В одной среде это может быть `AGENTS.md`, в другой — `CLAUDE.md`, набор навыков, профиль разрешений, [MCP][g-mcp]-ресурс или специальный шаблон запроса. Поставщикоспецифичные инструкции могут создаваться автоматически, но их содержание должно выводиться из [переносимого ядра][g-portable-core] и не становиться отдельным конкурирующим источником истины [26](https://developers.openai.com/codex/agent-configuration/agents-md).
...
При смене инструмента заменяется [адаптер исполнителя][g-executor-adapter]. Цели, решения, риски и текущее состояние проекта сохраняются.


Рисунок 1.7 — Переносимое ядро CHLOYA и адаптеры исполнителей.
...
Переносимость должна проверяться на практике. Новый исполнитель должен суметь объяснить цель проекта, определить разрешённую область, правильно прочитать [контракты][g-contract] и назвать действующие ограничения. При нехватке сведений он запрашивает конкретное расширение, а не скрыто восполняет пробелы предположениями.

Результат сохраняется в совместимом формате, а новые знания возвращаются в нейтральную [память проекта][g-project-memory]. Если при смене агента регулярно теряется важное ограничение, наличие одинаковых файлов само по себе не доказывает переносимость.
...
|---|---|
| Код спорной операции | Передаётся только среде, допущенной [политикой][g-policy] проекта. Закрытые инфраструктурные части при необходимости заменяются локальной выжимкой. |
| Ключи и платёжные токены | Исключаются из [контекста][g-context]. Разрешённые операции выполняются через секрет-брокер или тестовые значения. |
...
| Итог анализа | Сохраняется в нейтральной памяти вместе с [доказательствами][g-evidence], отклонёнными гипотезами и [остаточным риском][g-residual-risk]. |
...
Если этих сведений недостаточно, он указывает, какое конкретное поле или событие необходимо и почему. Исходные журналы не раскрываются автоматически: решение о расширении [контекста][g-context] принимает ответственное лицо либо доверенный локальный контур.

После завершения в [проектной памяти][g-project-memory] фиксируются причина ошибки, подтверждающие данные, отклонённые гипотезы, исправление, регрессионный тест, [остаточный риск][g-residual-risk] и сведения о [контексте][g-context], переданном внешнему исполнителю.
...
[Внешняя модель][g-external-model] получает только необходимый и разрешённый [контекст][g-context]. Условия обработки проверяются для конкретного продукта и конфигурации. Секреты заменяются ограниченными операциями, а чувствительная информация направляется в среду, допустимую для её класса.

[Локальные модели][g-local-model] также работают с минимальными [полномочиями][g-authority] и контролируемыми инструментами. Существенные решения, исправления и [доказательства][g-evidence] не остаются только в чатах, а преобразуются в нейтральные проектные артефакты.

Поставщикоспецифичные возможности изолируются в [адаптерах исполнителя][g-executor-adapter] и не заменяют [переносимое ядро][g-portable-core]. Способность продолжить работу проверяется практической сменой исполнителя, а собственная инфраструктура CHLOYA не должна превращаться в новую обязательную закрытую зависимость.
...
[g-context-on-demand]: ../../research/GLOSSARY.ru.md#контекст-по-запросу
...
[g-handoff]: ../../research/GLOSSARY.ru.md#handoff
[g-interaction-history]: ../../research/GLOSSARY.ru.md#история-взаимодействия
[g-intermediary-layer]: ../../research/GLOSSARY.ru.md#посреднический-слой
[g-local-change]: ../../research/GLOSSARY.ru.md#локальное-изменение
[g-local-model]: ../../research/GLOSSARY.ru.md#локальная-модель
[g-minimal-context]: ../../research/GLOSSARY.ru.md#минимально-достаточный-контекст
[g-model-aggregator]: ../../research/GLOSSARY.ru.md#агрегатор-моделей
[g-model-token]: ../../research/GLOSSARY.ru.md#токен-модели
[g-mcp]: ../../research/GLOSSARY.ru.md#mcp
[g-multi-agent-work]: ../../research/GLOSSARY.ru.md#многоагентная-работа
[g-policy]: ../../research/GLOSSARY.ru.md#политика
[g-portable-core]: ../../research/GLOSSARY.ru.md#переносимое-ядро
...
[g-stopping-condition]: ../../research/GLOSSARY.ru.md#условие-остановки
[g-task-context-capsule]: ../../research/GLOSSARY.ru.md#контекстная-капсула-задачи
