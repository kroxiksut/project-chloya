...
Глава 6 рассматривает CHLOYA не как заранее доказанную методологию, а как совокупность **проверяемых механизмов и гипотез**. Каждый механизм вводится для решения наблюдаемой проблемы агентной разработки, но одновременно способен создавать собственные накладные расходы, ограничения и новые режимы отказа.

Поэтому наличие контракта, проектной памяти, дополнительной проверки, маршрутизации моделей, нескольких исполнителей или защитного контура само по себе не считается преимуществом. Практическая ценность возникает только тогда, когда механизм даёт измеримый эффект, соответствующий его назначению, а этот эффект оправдывает стоимость применения.

Разделы 6.1–6.21 рассматривают риски разных уровней: ошибки постановки и локализации, человеческой и автоматической проверки, выбора моделей, передачи работы и многоагентной координации, доверия к внешнему контексту и зависимостям, переносимости инструментов и накопления проектной памяти. При этом многие риски связаны между собой. Ошибка раннего решения может распространиться через декомпозицию; недоверенное содержание — попасть в долговременную память; избыточная память — ухудшить рабочий контекст; ускорение генерации — увеличить нагрузку на проверку.

Общая логика главы:

> **наблюдаемая проблема → предполагаемый механизм CHLOYA → проверяемая гипотеза → измеримый результат → критерий пересмотра.**

Отрицательный результат является допустимым исходом. Если механизм не показывает практически значимой пользы, он должен упрощаться, становиться необязательным или исключаться. Если определённый профиль CHLOYA полезен только для части проектов и уровней риска, это определяет его область применимости, а не требует распространения на все задачи.

Таким образом, следующие разделы описывают не обещания CHLOYA, а **условия, при которых её основные положения могут быть подтверждены, ограничены или опровергнуты практикой**.

## Содержание

- [6.1. Бюрократизация управления: когда контроль начинает мешать работе](#61-бюрократизация-управления-когда-контроль-начинает-мешать-работе)
- [6.2. Локальное принятие решений и пределы самостоятельности](#62-локальное-принятие-решений-и-пределы-самостоятельности)
- [6.3. Центральный координатор становится узким местом](#63-центральный-координатор-становится-узким-местом)
- [6.4. Контракты отстают от кода](#64-контракты-отстают-от-кода)
- [6.5. Избыточное доверие к ИИ у нетехнического человека](#65-избыточное-доверие-к-ии-у-нетехнического-человека)
- [6.6. Человек становится узким местом](#66-человек-становится-узким-местом)
- [6.7. Разные модели по-разному понимают проектную память](#67-разные-модели-по-разному-понимают-проектную-память)
- [6.8. Утечка конфиденциальной информации](#68-утечка-конфиденциальной-информации)
- [6.9. Компрометация инструментального контура](#69-компрометация-инструментального-контура)
- [6.10. Разрушительные и труднообратимые действия](#610-разрушительные-и-труднообратимые-действия)
- [6.11. Тесты подтверждают неверную интерпретацию](#611-тесты-подтверждают-неверную-интерпретацию)
- [6.12. Конфликты интеграции и деградация истории изменений](#612-конфликты-интеграции-и-деградация-истории-изменений)
- [6.13. Избыточная документация](#613-избыточная-документация)
- [6.14. Контракт как граница исполнения, а не поиска решения](#614-контракт-как-граница-исполнения-а-не-поиска-решения)
- [6.15. Ошибочная маршрутизация моделей и вычислительного усилия](#615-ошибочная-маршрутизация-моделей-и-вычислительного-усилия)
- [6.16. Зависимость от собственного инструментария и потеря переносимости](#616-зависимость-от-собственного-инструментария-и-потеря-переносимости)
- [6.17. Разрыв между методологической стройностью и практической полезностью](#617-разрыв-между-методологической-стройностью-и-практической-полезностью)
- [6.18. Нарушение доверительной границы между контекстом, полномочием и действием](#618-нарушение-доверительной-границы-между-контекстом-полномочием-и-действием)
- [6.19. Доверие к новой версии зависимости и продвижение внешних артефактов](#619-доверие-к-новой-версии-зависимости-и-продвижение-внешних-артефактов)
- [6.20. Каскадирование ошибок координации и передачи работы](#620-каскадирование-ошибок-координации-и-передачи-работы)
- [6.21. Рост проектной памяти и деградация рабочего контекста](#621-рост-проектной-памяти-и-деградация-рабочего-контекста)
...
Практическая книга Albada также отдельно подчёркивает значимость происхождения и целостности данных в агентных системах, а при использовании инструментов с изменяемым состоянием рекомендует ограничивать доступные операции и применять принцип минимальных полномочий [194]. Эти рекомендации совпадают с уже сформированным в CHLOYA разделением анализа, разрешения и исполнения.
...
## 6.19. Доверие к новой версии зависимости и продвижение внешних артефактов

Использование внешней зависимости предполагает доверие не только к её назначению и интерфейсу, но и к конкретному коду, который фактически попадает в проект. При этом накопленная репутация библиотеки или её сопровождающих легко переносится на новую версию автоматически: если предыдущие выпуски использовались годами без серьёзных проблем, следующий выпуск психологически воспринимается как продолжение уже известного объекта.

С инженерной точки зрения это не вполне корректно. Новый выпуск может содержать иной исходный код, новые транзитивные зависимости, изменённые сценарии установки и сборки, новые бинарные компоненты или изменения процесса публикации. Даже при сохранении прежних сопровождающих в него может попасть критический дефект. При компрометации учётной записи, токена публикации или части цепочки сборки новый выпуск способен содержать уже намеренно вредоносный код.

Поэтому CHLOYA различает **доверие к внешнему проекту** и **принятие конкретного выпуска**.

> **Доверие к внешнему проекту не означает автоматического доверия к каждой его новой версии.**

Репутация, зрелость сообщества и история использования являются полезными сигналами, но решение должно относиться к конкретному программному артефакту.

### Публикация и принятие — разные события

Появление новой версии во внешнем реестре означает только то, что соответствующий артефакт стал доступен. Это ещё не означает, что он проверен конкретным проектом, соответствует его политике, допустим для производственной среды или должен немедленно заменить ранее принятую версию.

CHLOYA поэтому разделяет **публикацию внешнего артефакта** и **его продвижение в доверенный контур проекта**.

```mermaid
flowchart TB
    A["Новый внешний артефакт<br/>ещё не принят проектом"]
    B["Период наблюдения"]
    C["Проверка версии, цепочки и происхождения"]
    D["Изолированное испытание"]
    E{"Решение"}
    F["Отклонить или отложить"]
    G["Продвижение"]
    H["Утверждённый источник"]
    I["Использование проектом"]
...
**Рисунок 6.19.1 — Продвижение нового внешнего артефакта в доверенный контур проекта.**

Схема не означает, что каждая зависимость должна проходить одинаково тяжёлую процедуру. Глубина проверки зависит от риска. Существен сам переход: внешний реестр предоставляет **кандидата**, а не автоматически доверенный компонент проекта.

### Доверие относится к конкретному артефакту

Даже сочетание имени пакета и номера версии не всегда полностью определяет исполняемый код. Один выпуск может содержать архив исходного кода и несколько бинарных сборок для различных операционных систем, архитектур или версий среды выполнения. В некоторых экосистемах к уже существующему выпуску исторически можно было добавлять новые файлы.

Поэтому для значимых зависимостей идентичность принятого компонента следует определять точнее:

> **пакет + версия + источник + конкретный артефакт + контрольная сумма + доступные сведения о происхождении.**

Актуальный пример даёт изменение политики PyPI в июле 2026 года. Реестр начал запрещать добавление новых файлов к выпускам старше четырнадцати дней. Один из рассматривавшихся рисков состоял в том, что после компрометации публикационных полномочий злоумышленник мог добавить новый вредоносный бинарный артефакт к старой версии, которой пользователи уже привыкли доверять [339].

Отсюда следует важное различие:

> **номер версии идентифицирует выпуск, но сам по себе не доказывает идентичность фактически полученного артефакта.**

### Происхождение — свидетельство, но не гарантия безопасности

SLSA рассматривает происхождение программного артефакта как проверяемую информацию о том, откуда он появился и каким процессом был произведён [12]. Для CHLOYA такие сведения полезны, поскольку позволяют связать установленный объект с исходной ревизией, процессом сборки и конкретным опубликованным файлом.

Однако происхождение отвечает прежде всего на вопрос **«откуда это появилось?»**, а не **«безопасно ли это?»**. Легитимный сопровождающий способен случайно внести критический дефект. Вредоносное изменение может находиться уже в корректно аттестованном исходном коде. Поэтому наличие подтверждённого происхождения не отменяет анализа содержимого и поведения.

То же относится к lock-файлу и контрольной сумме. Lock-файл фиксирует ожидаемый набор версий. Контрольная сумма позволяет проверить, что получен тот же артефакт. Но контрольная сумма столь же хорошо закрепляет вредоносный файл, если именно он был однажды принят.

В CHLOYA это является частным случаем более общего антипаттерна **ложной гарантии**: свидетельство одного свойства ошибочно принимается за доказательство другого, более сильного свойства.

### Почему задержка иногда является защитным механизмом

Инцидент с LiteLLM и Telnyx в PyPI в 2026 году хорошо показывает опасность немедленного принятия новых выпусков. После компрометации цепочки публикации были размещены версии с вредоносным кодом, выполнявшимся при установке и собиравшим чувствительные сведения [10]. Для LiteLLM от публикации вредоносных версий до карантина реестра прошло около двух с половиной часов, однако за это время они успели получить более ста тысяч загрузок.

Для агентной разработки этот риск дополнительно усиливается скоростью автоматизации. ИИ-исполнитель способен за минуты обнаружить новый выпуск, изменить описание зависимостей, пересчитать зафиксированную цепочку, скачать новый артефакт, выполнить установочные сценарии и продолжить работу уже с новым кодом.

То, что раньше частично задерживалось естественным человеческим процессом, может происходить почти мгновенно.

Поэтому CHLOYA вводит понятие **[защитной задержки][g-protective-delay]** — намеренного промежутка между появлением внешнего артефакта и его обычным допуском в доверенный контур.

> **Время само по себе не проверяет код. Оно даёт другим механизмам обнаружения возможность создать новые сведения о нём.**

За это время могут появиться сообщения исследователей, жалобы пользователей, отзыв версии, предупреждение реестра, сведения об уязвимости или исправляющий выпуск. Подобный механизм уже существует практически: например, `pnpm` поддерживает минимальный возраст версии перед её установкой и применяет это правило также к транзитивным зависимостям [11].

### Десять суток не должны быть универсальной константой

В предыдущей версии CHLOYA использовалось рабочее правило десятидневной выдержки для новых прямых и транзитивных версий.

Сейчас его правильнее трактовать как **исходное значение экспериментального профиля**, а не как неизменное правило всей методологии.

Различные экосистемы отличаются скоростью публикации, частотой обновлений, качеством реестров, типичными сроками обнаружения атак и стоимостью задержки. Кроме того, значение риска зависит от самого проекта. Поэтому CHLOYA должна требовать наличия явно определённой политики [периода наблюдения][g-observation-period], но конкретную продолжительность следует подбирать и затем проверять экспериментально.

Таким образом, исследовательский вопрос меняется. Проверять нужно не утверждение «десять дней безопасны», а более общий принцип: **даёт ли риск-зависимая задержка измеримый выигрыш относительно немедленного принятия новых выпусков и какой ценой достигается этот выигрыш**.

### Задержка не должна препятствовать срочным исправлениям

Механическое ожидание создаёт противоположный риск. Если используемая версия содержит известную критическую уязвимость, а новый выпуск её исправляет, многодневный запрет на обновление способен сделать систему менее безопасной.

Поэтому в CHLOYA должны существовать два пути принятия.

Обычный путь использует [период наблюдения][g-observation-period] и стандартный набор проверок. Срочный путь применяется, когда риск продолжения работы на текущей версии оценивается выше риска принятия свежего выпуска. В таком случае [период наблюдения][g-observation-period] может сокращаться, но это должно компенсироваться более глубокой непосредственной проверкой, изолированным испытанием и явной фиксацией основания решения.

> **Сокращение времени наблюдения должно компенсироваться усилением непосредственных свидетельств.**

Срочное принятие поэтому не является отключением безопасности. Оно представляет собой другой профиль той же политики.

### Свежая и устаревшая версия создают разные риски

Более старая версия также не является автоматически более безопасной. С течением времени в ней могут обнаруживаться уязвимости, прекращаться сопровождение или появляться несовместимости с остальной системой.

Политика должна балансировать два противоположных риска: недостаточно наблюдавшийся новый выпуск и уже устаревающий старый выпуск.

Поэтому CHLOYA не стремится к максимальному возрасту зависимости. Цель состоит в выборе **допустимого момента принятия**, когда [период наблюдения][g-observation-period] уже дал дополнительные внешние сведения, но проект не удерживается без необходимости на известной уязвимой версии.

Это превращает возраст выпуска из бинарного правила в параметр управления риском.

### Транзитивная зависимость является таким же внешним кодом

Доверие нельзя проверять только для зависимостей, явно записанных разработчиком.

Обновление одной библиотеки способно изменить несколько уровней транзитивной цепочки. Возможна и обратная ситуация: прямые версии остаются прежними, а изменение правил разрешения зависимостей или lock-файла приводит к появлению нового кода глубже в графе.

Поэтому:

> **Новый транзитивный выпуск является таким же новым внешним кодом, как новая прямая зависимость.**

[Период наблюдения][g-observation-period] и другие существенные ограничения должны применяться к реально устанавливаемому графу, а не только к верхнему уровню файла зависимостей.

### Обновление — это изменение цепочки, а не одной строки
...
может скрывать значительно более широкий набор изменений. Помимо собственного исходного кода библиотеки может измениться её транзитивная цепочка, сценарии установки, бинарные компоненты, права, сетевое поведение, процесс сборки или набор сопровождающих.

После раздела 6.18 сюда добавляется ещё один класс риска: пакет может приносить в проект не только исполняемый код, но и управляющие артефакты агентной среды — инструкции, навыки, конфигурации средств разработки и другие файлы, влияющие на поведение ИИ-исполнителя.

Следовательно, цепочка поставки агентной разработки включает **как исполняемые, так и управляющие внешние артефакты**.

### Исходный код и устанавливаемый артефакт не всегда совпадают

Просмотр изменений в исходном репозитории является полезной, но не полной проверкой.

Публикуемый пакет может собираться отдельным процессом и содержать данные, которых нет в просмотренном дереве исходного кода. Поэтому для значимых зависимостей полезно сохранять связь:

> **исходная ревизия → процесс сборки → сведения о происхождении → опубликованный артефакт → контрольная сумма → фактически установленная копия.**

Чем выше риск, тем более полной должна быть эта цепочка свидетельств.

### Внутреннее зеркало как граница продвижения

Внутреннее зеркало в этой модели выполняет не только роль кэша или ускорителя загрузки.

Оно может стать технической реализацией **[границы продвижения][g-promotion-boundary]**.

До проверки внешний артефакт является кандидатом. После принятия конкретная копия с зафиксированной контрольной суммой попадает в утверждённый источник, а производственные сборки получают зависимости уже оттуда.

Такой подход уменьшает вероятность того, что одна и та же декларация зависимостей в разные моменты времени приведёт к получению разных внешних артефактов.

При этом само зеркало становится частью доверенного вычислительного контура. Его права публикации, целостность и журналирование также требуют защиты.

### Проверку можно автоматизировать, доверие — только в пределах заранее заданной политики

ИИ-агент способен существенно снизить стоимость анализа обновлений. Он может обнаружить новый выпуск, сравнить изменения, исследовать граф зависимостей, проверить [период наблюдения][g-observation-period], изучить сведения о происхождении, запустить статический анализ и тесты в изолированной среде.

Но здесь сохраняется принцип предыдущего раздела:

> **анализ не создаёт полномочие.**

Исполнитель, исследовавший зависимость, не должен автоматически получать право снять ограничение только потому, что сам признал результат безопасным.

Для высокого риска его вывод становится частью свидетельств, после чего обязательные условия проверяются формальной политикой и, если требуется, уполномоченным человеком или отдельным контуром принятия.

В низкорисковом режиме политика может заранее разрешать полностью автоматическое продвижение после прохождения установленных условий. Таким образом, CHLOYA требует не обязательного ручного решения, а **предварительно определённого источника полномочия на принятие**.

### Доверие ограничено областью применения

Принятие внешнего артефакта также не обязано быть глобальным.

Версия может быть допущена в исследовательской или тестовой среде, но ещё не в производственной. Один бинарный артефакт может быть проверен для Linux, но это не означает автоматического доверия к другой сборке той же версии для Windows. Компонент может быть принят одним проектом, но не всеми проектами организации.

Поэтому доверие должно относиться к конкретному сочетанию артефакта и области применения.

> **Принятие в одной области не распространяется автоматически на другие области.**

Это продолжает более общий принцип CHLOYA: доверие и полномочия должны сохранять исходную область действия.

### Защитная задержка особенно важна при высокой скорости автоматизации

ИИ сокращает естественное трение процесса разработки. Обычно это полезно, но в цепочке поставки некоторое трение исторически давало внешней экосистеме время обнаружить проблему.

Поэтому не всякое уменьшение задержки автоматически является улучшением.

> **Не всякое трение является неэффективностью. Иногда намеренная задержка представляет собой независимую защитную границу.**

Ценность такой границы заключается в том, что она не зависит от способности конкретной модели обнаружить вредоносное изменение. Даже если агент признал выпуск безопасным, артефакт может оставаться вне обычного пути принятия до завершения установленного периода.

### Принцип применим не только к библиотекам

Та же модель пригодна для контейнерных образов, компонентов CI/CD, провайдеров Terraform, коллекций Ansible, плагинов, бинарных инструментов, внешних моделей и подключаемых навыков ИИ-агентов.

Общий переход остаётся одинаковым:

> **внешний изменяемый артефакт → кандидат → проверка → явное принятие → доверенный контур.**

Различаются только конкретные способы проверки и последствия ошибки.

### Исследовательская гипотеза

**H6.19.** Разделение публикации внешней версии и её продвижения в доверенный контур проекта, включающее риск-зависимый [период наблюдения][g-observation-period], фиксацию конкретного артефакта, проверку происхождения и транзитивной цепочки и отдельный порядок срочного принятия, снижает вероятность попадания скомпрометированного или иного нежелательного выпуска в доверенную сборку по сравнению с немедленным автоматическим обновлением при приемлемой задержке установки легитимных обновлений безопасности.

Дополнительная гипотеза:

**H6.19a.** Риск-зависимый [период наблюдения][g-observation-period] обеспечивает лучшее соотношение между защитой от проблем свежих выпусков и своевременным устранением известных уязвимостей, чем единый фиксированный период ожидания для всех зависимостей.

### Экспериментальная проверка

Проверку можно проводить без публикации вредоносных компонентов во внешние реестры. Для этого создаётся локальный контролируемый реестр с последовательностью выпусков, среди которых присутствуют обычные, скомпрометированные и срочно исправляющие известную уязвимость.

Первый режим использует немедленное принятие последней совместимой версии. Второй использует только фиксацию зависимостей, но позволяет средству обновления сразу сформировать новый зафиксированный набор. Третий применяет политику CHLOYA с периодом наблюдения, проверкой конкретных артефактов, происхождения и цепочки зависимостей. Отдельный режим моделирует срочное исправление, когда обычный период ожидания сокращается и компенсируется усиленной проверкой.

В набор сценариев следует включать как прямые, так и транзитивные изменения, новые сценарии установки, бинарные артефакты, изменение публикационных полномочий, расхождение между исходным кодом и собранным пакетом, а также обычные безопасные обновления. Такой дизайн позволяет измерять не только способность остановить вредоносный выпуск, но и цену защитной политики.

### Показатели оценки

| Группа            | Основные показатели                                                                                                             |
| ----------------- | ------------------------------------------------------------------------------------------------------------------------------- |
| Безопасность      | Доля нежелательных артефактов, достигших доверенной сборки; время потенциального воздействия; число обходов политики            |
| Своевременность   | Задержка обычных обновлений; задержка исправлений безопасности; время использования известной уязвимой версии                   |
| Воспроизводимость | Совпадение с утверждёнными контрольными суммами; незапланированные изменения транзитивной цепочки                               |
| Управляемость     | Доля принятых артефактов с установленным происхождением и основанием решения; использование утверждённых источников             |
| Стоимость         | Человеческое время, вычислительные затраты, число срочных исключений и безопасных выпусков, задержанных без практической пользы |

Безопасность и своевременность должны рассматриваться совместно. Политика, запрещающая все обновления, способна показать отличный результат по проникновению свежих вредоносных выпусков и одновременно оставить проект на известных критических уязвимостях. Такая политика не считается успешной.

### Объект и предмет исследования

**Объект исследования** — процесс принятия и обновления внешних программных и управляющих артефактов в агентной разработке.

**Предмет исследования** — влияние [периода наблюдения][g-observation-period], проверки конкретного артефакта, происхождения, транзитивной цепочки и явного продвижения на вероятность попадания нежелательного внешнего кода в доверенный контур и на своевременность легитимных обновлений.

### Угрозы валидности

Экосистемы значительно различаются по скорости обнаружения вредоносных выпусков, поэтому результаты для PyPI нельзя непосредственно переносить на npm, контейнерные реестры или другие системы. Реальные компрометации популярных компонентов относительно редки, из-за чего лабораторный эксперимент хорошо проверяет механизм защиты, но значительно хуже оценивает естественную вероятность самой атаки.

[Период наблюдения][g-observation-period] также легко переоценить. Вредоносная версия может оставаться необнаруженной дольше выбранного срока. Поэтому выдержка уменьшает определённый класс риска, но не является доказательством безопасности.

Противоположную угрозу создаёт чрезмерная задержка обновлений. Слишком консервативная политика способна увеличить время эксплуатации уже известной уязвимости. Аналогично происхождение, подпись или контрольная сумма могут быть ошибочно восприняты как доказательство безопасности содержимого, хотя подтверждают только более узкие свойства.

Автоматический анализ модели также не является безошибочным: агент способен пропустить вредоносное изменение или ошибочно заблокировать безопасное. Поэтому его заключение рассматривается как свидетельство, а не как абсолютная гарантия.

Наконец, внутреннее зеркало после превращения в доверенный источник само становится важной точкой атаки и должно входить в защищаемый контур.

### Граница применимости

CHLOYA не требует многодневной ручной экспертизы каждой новой библиотеки.

Для низкорисковых компонентов может быть достаточна автоматическая проверка с коротким периодом наблюдения. Для критичных зависимостей могут потребоваться более глубокий анализ, изолированное выполнение и отдельное решение о продвижении.

Не утверждается и то, что новая версия в принципе опаснее старой. Свежий выпуск может быть единственным способом устранить критическую уязвимость.

Поэтому раздел не устанавливает максимальную осторожность как самоцель.

Его основной принцип:

> **Публикация нового внешнего артефакта и решение проекта доверять этому артефакту являются разными событиями.**

Второй принцип:

> **Доверие назначается конкретному артефакту и области применения и не наследуется автоматически от имени пакета, репутации проекта или предыдущей версии.**

И третий:

> **[Защитная задержка][g-protective-delay] покупает время для появления дополнительных свидетельств, но не заменяет проверку происхождения, содержимого и последствий обновления.**


[↑ К содержанию](#содержание)

## 6.20. Каскадирование ошибок координации и передачи работы

Разделение сложной задачи между несколькими исполнителями позволяет использовать специализацию и параллельность, но одновременно создаёт новый класс отказов. Ошибка одного локального исполнителя обычно ограничена его областью до тех пор, пока результат не становится основанием для последующей работы. Ошибка, возникшая раньше — при понимании исходной цели, выборе архитектурного предположения, декомпозиции или передаче задачи, — способна распространиться сразу на несколько ветвей.

В такой ситуации каждый отдельный исполнитель может действовать последовательно и технически качественно, но все они будут реализовывать одну и ту же ошибочную предпосылку.

> **Многоагентная система способна не только параллельно выполнять работу, но и параллельно размножать ошибку.**

Исследование Cemri et al. показывает, что отказы многоагентных систем не сводятся к качеству отдельных моделей. Выявленные авторами режимы отказа относятся к проектированию системы, рассогласованию между агентами и недостаточности проверки результата [21]. Улучшение только описания ролей или схемы координации не устраняло выявленные проблемы полностью.

Для CHLOYA отсюда следует, что функциональные роли и формализованная передача работы необходимы, но сами по себе не являются достаточной защитой от каскадирования ошибок.

### Локальная ошибка и ошибка до разветвления имеют разный радиус последствий

Если исполнитель ошибается внутри независимого модуля и его результат проверяется до использования другими участниками, последствия могут оставаться локальными.

Иная ситуация возникает, если до начала параллельной работы координатор ошибочно решает, например, что определённый интерфейс можно изменить без сохранения совместимости. Затем одна ветвь изменяет производителя данных, вторая — потребителя, третья — тесты, а четвёртая — документацию. Все исполнители могут правильно выполнить полученные задания, но вся совокупность изменений будет исходить из ошибочного решения.

Таким образом, значение имеет не только вероятность ошибки, но и **место её возникновения в графе зависимостей работы**.

Работа Xie et al. рассматривает многоагентное взаимодействие именно как направленный граф зависимостей и показывает, что локальная ошибка может усиливаться по мере распространения между участниками, а структура взаимодействия существенно влияет на итоговый масштаб отказа [341].

Для CHLOYA это позволяет сформулировать общий принцип:

> **Чем больше последующих работ принимают некоторое решение или предположение за исходное основание, тем выше потенциальный радиус его ошибки.**

Поэтому одинаковая по содержанию ошибка может иметь совершенно разные последствия в зависимости от того, появилась она в конечной ветви или перед крупным разветвлением работ.

### Ошибка может возникать на разных уровнях координации

Каскад не следует связывать только с центральным координатором.

Ошибка может появиться уже при интерпретации первоначального запроса. Она может возникнуть при декомпозиции, когда правильная цель разбивается на неподходящие подзадачи. Правильная декомпозиция способна исказиться при передаче, если из локального пакета исчезло существенное условие. Наконец, отдельный исполнитель может создать ошибочный результат, который другие ветви начинают использовать как подтверждённое основание.

Возможен и другой класс отказа: все локальные результаты корректны по отдельности, но при объединении оказываются несовместимыми или не реализуют исходную цель.

Поэтому 6.20 рассматривает **каскадирование координационной ошибки**, а не только «ошибку оркестратора».

Особенно важна граница передачи работы. Работа AgentAsk показывает, что ошибки многоагентного взаимодействия часто возникают непосредственно на таких границах. Авторы выделяют потерю необходимых данных, искажение передаваемого сигнала, изменение смысла ссылок на объекты и несоответствие возможностей получателя требованиям задачи [340].

Это хорошо согласуется с уже используемым в CHLOYA представлением передачи работы не как пересылки произвольного текста, а как контролируемого преобразования состояния задачи.

### Проверка перед разветвлением

Одинаковая проверка имеет разную ценность в разных точках процесса.

Предположение, от которого зависит одна обратимая локальная правка, обычно не требует того же уровня контроля, что предположение, после которого будут запущены десятки взаимосвязанных задач.

Поэтому CHLOYA вводит **проверку перед разветвлением**.

> **Решение, которое станет общей предпосылкой для большого числа зависимых работ, должно проверяться до масштабного запуска этих работ пропорционально неопределённости решения и ожидаемым последствиям ошибки.**

Это не означает обязательного человеческого согласования перед каждым созданием подзадачи.

Проверка может состоять в сопоставлении с исходными требованиями, выполнении детерминированной проверки, дополнительном исследовании спорной зависимости, получении второго действительно независимого заключения или запросе решения человека для высокорисковой развилки.

Существенно другое: дорогостоящая параллельность не должна запускаться только потому, что координатор способен быстро породить большое количество формально хорошо оформленных заданий.

Этот принцип связан с ранее выделенным в CHLOYA **усилением ранней ошибки**: недостаточно проверенное решение на ранней стадии способно размножить стоимость на архитектуру, реализацию, тестирование и интеграцию последующих стадий.

### Подзадача должна сохранять связь с исходной целью

Локализация контекста требует сокращать передаваемую информацию, но сокращение не должно разрывать смысловую связь между подзадачей и причиной её существования.

Исполнителю не обязательно получать всю историю проекта или исходный разговор. Однако для существенной работы должна оставаться восстанавливаемая связь между локальной задачей, исходной целью, ключевыми решениями и обязательными инвариантами.

Например, производная задача может быть связана с исходной целью и решением следующим образом:

```text
TASK-27
реализует: GOAL-4
основана на: DECISION-12
обязательные инварианты: INV-3, INV-8
существенное предположение: ASSUMPTION-5
```

Такая трассируемость нужна не ради дополнительной документации. Она позволяет при обнаружении ошибки понять, **какие последующие работы зависят от ошибочного основания**.

Если `ASSUMPTION-5` признано неверным, управляющий контур получает возможность определить затронутые задачи и решения, а не искать последствия вручную по всему проекту.

Это также продолжает сформулированный ранее принцип сохранения происхождения: происхождение существенного утверждения не должно исчезать только потому, что оно было преобразовано в локальную постановку задачи.

### Полномочие координатора не делает его предположение истинным

Координатор может иметь законное право распределять работу и определять область локальной задачи. Однако организационное полномочие не превращает его техническое предположение в достоверный факт.

Поэтому исполнитель не должен рассматриваться как пассивный механизм выполнения постановки.

Если локальные данные показывают противоречие между контрактом задачи и фактическим устройством системы, исполнитель должен иметь право остановить работу и оспорить исходную предпосылку.

Здесь 6.20 непосредственно опирается на механизм **оспаривания контракта**, введённый в 6.14.

Право оспорить контракт не означает права самостоятельно изменить решение более высокого уровня. Оно означает право вернуть обнаруженное противоречие туда, где существует полномочие пересмотреть соответствующее решение.

Таким образом, CHLOYA разделяет две ситуации:

> **локальная ошибка требует локального исправления;**

> **ошибка общей предпосылки требует пересмотра зависимого от неё фрагмента координации.**

### Ошибка должна распространять сигнал обратно вверх

В устойчивой многоагентной системе информация движется не только от координатора к исполнителям и затем в виде готовых результатов обратно.

Исполнитель способен обнаружить сведения, которые делают ошибочным само родительское решение.

```mermaid
flowchart TB
    G["Исходная цель"]
    V["Проверка общих предпосылок"]
    D["Декомпозиция"]
    ...
    I["Интеграция"]
    F["Проверка относительно исходной цели"]

    X["Обнаружена ошибка<br/>общей предпосылки"]
    R["Пересмотр родительского решения"]
    S["Остановка или пометка<br/>зависимых ветвей"]
...
**Рисунок 6.20.1 — [Проверка перед разветвлением][g-pre-branching-verification] и обратное распространение сигнала об ошибке общей предпосылки.**

Если исполнитель обнаружил локальную ошибку собственной реализации, нет необходимости останавливать независимые ветви.

Если же выясняется, что ошибочно общее решение, управляющий контур должен уметь определить связанные с ним работы, приостановить дальнейшее распространение ошибки и инициировать новую декомпозицию.

Такой обратный путь особенно важен при высокой степени параллельности. Без него одна ветвь может уже доказать ошибочность исходной предпосылки, пока остальные продолжают расходовать ресурсы на её реализацию.

### Радиус распространения ошибки

Для оценки каскада полезно рассматривать не только факт наличия дефекта, но и его **радиус распространения**.

Под радиусом понимается область работ и артефактов, которые успели принять ошибочную предпосылку как основание для дальнейших действий.

У ошибки, обнаруженной до запуска зависимых работ, радиус может быть близок к одной точке. Та же ошибка после нескольких передач способна затронуть множество задач, тестов, документов, решений и элементов проектной памяти.

Радиус не обязательно выражать одним универсальным числом. В эксперименте его можно характеризовать количеством затронутых ветвей, глубиной распространения, числом производных артефактов и объёмом работы, требующей пересмотра.

Особенно опасно попадание ошибки в каноническую память проекта. В этом случае каскад перестаёт быть ограниченным текущей декомпозицией и может продолжиться уже в будущих задачах.

### Согласие нескольких исполнителей не всегда является независимым подтверждением

Дополнительные агенты часто используются как средство проверки: если несколько исполнителей пришли к одинаковому выводу, результат кажется более надёжным.

Но это справедливо только при достаточной независимости оснований.

Если три агента получили одну и ту же ошибочную постановку, одинаковую сокращённую память и одну ошибочную архитектурную предпосылку, их согласие может быть всего лишь многократным воспроизведением одной ошибки.

Исследование Kostka и Chudziak показывает близкую проблему при многоагентной проверке фактов: согласие между агентами способно создавать ложную уверенность при коррелированных ошибках [342].

Поэтому:

> **Количество согласных исполнителей не равно количеству независимых свидетельств.**

Если целью проверки является обнаружение ошибки декомпозиции, проверяющий должен иметь возможность сопоставить локальный результат с более первичным основанием — исходной целью, инвариантами, независимым источником данных или отдельным методом проверки.

Проверка, которая получает только тот же производный пакет, что и исполнитель, хорошо обнаруживает часть локальных ошибок, но значительно хуже защищает от ошибки самого пакета.

Этот принцип связывает 6.20 с более общим **принципом независимости свидетельств** CHLOYA.

### Множитель повторной работы

Ранняя ошибка отличается ещё и экономическим эффектом.

Предположение может быть сформулировано координатором за несколько минут, но после его распространения пять исполнителей создадут код, тесты и документацию, а затем человек потратит дополнительное время на проверку и интеграцию.

Стоимость ошибки тогда определяется уже не стоимостью момента, в котором она возникла, а всей зависимой работой, потерявшей ценность.

Для экспериментальной оценки CHLOYA вводит **[множитель повторной работы][g-rework-multiplier]**:
...
где (C_{origin}) — стоимость работы, в которой возникла исходная ошибка, а (C_{invalidated}) — стоимость последующей работы, которую пришлось отменить, существенно переделать или повторно проверить вследствие этой ошибки.

Показатель не следует трактовать как универсальную характеристику качества системы. Его назначение — показать, насколько архитектура координации способна увеличивать экономические последствия раннего дефекта.

Для отдельных ошибок множитель способен значительно превышать единицу.

### Генерация не должна обгонять способность проверять

Высокая параллельность создаёт ещё одну опасность. Координатор способен инициировать новые ветви быстрее, чем система проверяет общие предпосылки и интегрирует уже завершённые результаты.

В таком режиме быстро растёт объём **непроверенной зависимой работы**.

Это частный случай ранее выделенного в CHLOYA обратного давления проверки: удешевление и ускорение генерации переносит узкое место в понимание, проверку и интеграцию.

Для многоагентной координации принцип можно уточнить:

> **Скорость разветвления работы не должна устойчиво превышать способность системы проверять критические общие предпосылки и интегрировать результаты уже запущенных ветвей.**

Иначе ошибочное решение успевает породить большой объём правдоподобной работы до того, как появляется возможность оценить его последствия.

Это не означает жёсткого ограничения числа агентов. Практическим механизмом может быть ограничение числа одновременно находящихся в работе зависимых задач, промежуточные точки интеграции или повышение требований к предварительной проверке при увеличении числа ветвей.

Таким образом, параллельность имеет не только вычислительный бюджет, но и **[бюджет непроверенной работы][g-unverified-work-budget]**.

### Координатор не является источником абсолютной истины

Многоагентная архитектура не должна предполагать универсальную иерархию, в которой координатор обязательно интеллектуально сильнее каждого исполнителя.

Координация требует собственного профиля возможностей: понимания системных зависимостей, декомпозиции, работы с неопределённостью и интеграции результатов.

Локальный исполнитель при этом может значительно глубже понимать собственную предметную область.

Поэтому роль координатора состоит прежде всего во владении процессом декомпозиции и интеграции, а не в безусловной истинности всех его технических предположений.

Книга Albada также подчёркивает, что добавление новых агентов одновременно создаёт дополнительную стоимость связи, координации и ресурсов и рекомендует принцип минимально достаточного числа участников [194].

CHLOYA поэтому не предполагает, что более сложная многоагентная структура автоматически лучше одного исполнителя.

> **Многоагентность должна использоваться там, где выгода специализации, разделения контекста или параллельности превышает стоимость и дополнительный риск координации.**

### Многоагентность как проверяемое архитектурное решение

Сравнение «один агент против нескольких» само по себе недостаточно.

Одному исполнителю может быть трудно охватить большую задачу из-за объёма контекста. Несколько исполнителей могут лучше локализовать знания и работать параллельно, но проигрывать из-за передачи, повторения ошибок и интеграционных затрат.

Поэтому CHLOYA рассматривает количество исполнителей и структуру их взаимодействия как параметры архитектуры, которые должны обосновываться классом задачи.

Практически это означает, что добавление очередной роли должно отвечать хотя бы на один реальный дефицит: необходимость иной компетенции, изоляции контекста, независимой проверки, владения отдельной областью или полезной параллельности.

Само наличие технической возможности запустить больше агентов таким основанием не является.

### Исследовательская гипотеза

**H6.20.** Проверка решений с большим числом зависимых ветвей до начала масштабной декомпозиции, сохранение связи подзадач с исходными целями и предпосылками, возможность обратной эскалации и интеграционная проверка относительно исходных инвариантов уменьшают радиус распространения смысловых ошибок и объём вызванной ими повторной работы по сравнению с многоагентной координацией, в которой подзадачи формируются и проверяются преимущественно локально.

Дополнительная гипотеза:

**H6.20a.** При прочих сопоставимых условиях увеличение числа параллельных ветвей без соответствующего усиления проверки общей предпосылки увеличивает ожидаемую стоимость ошибки этой предпосылки быстрее, чем растёт стоимость её первоначального возникновения.

Вторая гипотеза требует особенно осторожной экспериментальной проверки: нелинейный характер роста нельзя считать доказанным заранее.

### Экспериментальная проверка

Пилот должен различать место возникновения ошибки.

В одной серии исходная цель остаётся корректной, но до декомпозиции в систему вводится неверное общее предположение. В другой декомпозиция остаётся корректной, а существенное условие теряется при передаче одной из подзадач. В третьей ошибку создаёт один локальный исполнитель, и исследуется её дальнейшее использование другими участниками. Отдельная серия должна проверять ситуацию, когда локальные результаты по отдельности приемлемы, но конфликтуют при интеграции.

Сравниваются три базовых режима.

**Режим A** использует одного исполнителя без многоагентного разветвления.

**Режим B** использует обычную многоагентную декомпозицию с локальными постановками и проверками.

**Режим C** использует контур CHLOYA: проверку критических предпосылок перед крупным разветвлением, трассируемую связь подзадач с исходной целью, возможность оспаривания постановки, распространение сигнала об ошибке к родительскому решению, остановку зависимых ветвей и итоговую проверку относительно исходных инвариантов.

Модели, задачи и доступные инструменты между режимами по возможности должны оставаться сопоставимыми. Иначе влияние методологии будет трудно отделить от различий исполнителей.

### Показатели оценки

| Группа                 | Основные показатели                                                                    |
| ---------------------- | -------------------------------------------------------------------------------------- |
| Распространение ошибки | Число затронутых ветвей; глубина распространения; число производных артефактов         |
| Обнаружение            | Время и стадия первого обнаружения; доля общих ошибок, остановленных до разветвления   |
| Повторная работа       | Объём отменённой или переделанной работы; [множитель повторной работы][g-rework-multiplier]                   |
| Проверка               | Доля общих ошибок, пропущенных локальными проверками; независимость оснований проверки |
| Координация            | Стоимость передачи, уточнений, остановок и повторной декомпозиции                      |
| Практический результат | Качество принятого результата, полная стоимость и время до его принятия                |

Особенно важно измерять не только количество ошибок, но и их последствия.

Система, допускающая несколько локальных быстро обнаруживаемых ошибок, может быть практически надёжнее системы с меньшим числом ошибок, одна из которых успевает распространиться на весь проект.

### Объект и предмет исследования

**Объект исследования** — координация связанных задач между одним или несколькими ИИ-исполнителями в программной разработке.

**Предмет исследования** — влияние места возникновения ошибки, структуры зависимостей, [проверки перед разветвлением][g-pre-branching-verification], трассируемости предпосылок и обратной эскалации на распространение ошибок и стоимость последующей переделки.

### Угрозы валидности

Существенной угрозой является искусственность вводимых ошибок. Слишком очевидная ложная предпосылка может сделать защитный механизм значительно эффективнее, чем он будет в реальной разработке. Ошибки для пилота должны быть правдоподобными и по возможности основываться на реально наблюдавшихся классах недопонимания требований, интерфейсов и зависимостей.

Вторая угроза связана с различием моделей. Сильный локальный исполнитель может самостоятельно обнаружить ошибку координатора, тогда как более слабый её воспроизведёт. Поэтому результат нельзя считать свойством только архитектуры без фиксации используемых моделей.

Третья угроза возникает при измерении независимости проверки. Два разных агента могут выглядеть независимыми организационно, но использовать одну модель, один контекст и одну ошибочную предпосылку. В таком случае их ошибки могут оставаться сильно коррелированными [342].

Четвёртая угроза — цена чрезмерной предварительной проверки. Если каждую незначительную развилку проверять так же тщательно, как системное решение с десятками зависимых ветвей, стоимость координации может уничтожить преимущество параллельности.

Пятая угроза связана с остановкой зависимых работ. Слишком агрессивное распространение сигнала способно приостанавливать ветви, которые фактически не зависят от ошибочной предпосылки. Поэтому система должна сохранять реальные зависимости, а не исходить из того, что любое изменение родительского решения аннулирует всю работу ниже него.

Наконец, успешность конкретной многоагентной схемы не следует автоматически переносить на другие классы задач. Исследования многоагентных систем показывают значительную зависимость результата от структуры взаимодействия, постановки и механизма проверки [21], [340], [341].

### Граница применимости

CHLOYA не утверждает, что любую сложную задачу следует разбивать между несколькими исполнителями.

Для локальной задачи один исполнитель может оказаться дешевле, быстрее и надёжнее, поскольку исключает стоимость передачи и часть координационных отказов.

Не требуется также проверять каждое решение перед каждой локальной ветвью. Усиленная проверка оправданна прежде всего там, где ошибка способна стать общей предпосылкой для значительного объёма дальнейшей работы.

Основной принцип раздела:

> **Риск решения определяется не только вероятностью того, что оно ошибочно, но и объёмом последующей работы, которая будет считать его истинным.**

Из этого следует второй принцип:

> **Чем шире предполагаемое разветвление зависимой работы, тем раньше должна происходить проверка наиболее значимых общих предпосылок.**

И третий:

> **Обнаружение ошибки общей предпосылки должно уметь распространяться обратно к её источнику и останавливать только действительно зависимые работы.**

Таким образом, цель CHLOYA состоит не в устранении любой ошибки координатора или исполнителя. Практически более достижимая задача — **ограничить способность одной ошибки незаметно превратиться в множество согласованных, дорогостоящих и внешне правдоподобных результатов**.
...
## 6.21. Рост проектной памяти и деградация рабочего контекста

Одной из задач CHLOYA является сохранение состояния проекта вне отдельных сеансов работы, внутренней памяти модели и конкретного инструмента. По мере развития проекта в памяти накапливаются решения, контракты, ограничения, результаты проверок, сведения о рисках, причины предыдущих изменений и другие знания, необходимые для продолжения работы.

Однако успешное сохранение этих сведений создаёт собственный риск. Если накопленная память начинает без отбора передаваться каждому следующему исполнителю, механизм, предназначенный для сохранения контекста, сам становится причиной контекстной перегрузки. Стоимость работы растёт, актуальные сведения конкурируют с историческими, а увеличение контекстного окна лишь откладывает проявление проблемы.

Поэтому CHLOYA различает **полноту проектной памяти** и **полноту рабочего контекста**.

> **Проектная память должна быть достаточно полной и восстанавливаемой. Рабочий контекст должен быть локальным, актуальным, доверенным и минимально достаточным.**

Такое разделение уже является частью общей архитектуры контекста CHLOYA: память проекта содержит потенциально необходимое знание, тогда как активный контекст представляет только ту его часть, которая действительно требуется для текущего шага работы.

### Проектная память не является активным контекстом

Большой долгоживущий проект может содержать сотни архитектурных решений, тысячи завершённых задач и значительный объём исторических сведений. Само наличие такого массива не является дефектом. Проблема возникает, если для локального изменения исполнитель вынужден каждый раз повторно обрабатывать значительную его часть.

CHLOYA поэтому рассматривает память как **источник формирования контекста**, а не как готовый контекст.

Условно путь знания можно представить так:

```text
полная проектная память
        ↓
доступное для поиска знание
        ↓
проекция под конкретную задачу
        ↓
активный рабочий контекст
```

Каждый следующий уровень является более узким представлением предыдущего. Сужение при этом не означает уничтожения более полного источника.

> **Сохранить знание не означает передать его каждому исполнителю при каждой задаче.**

И наоборот, отсутствие некоторого сведения в активном контексте не означает, что проект его забыл. Оно может оставаться доступным и подключаться при появлении соответствующей необходимости.

### Большое контекстное окно не устраняет проблему отбора

Техническая возможность передать модели всё больший объём текста не доказывает способность одинаково надёжно использовать каждую его часть.

Liu et al. в *Lost in the Middle* показали зависимость результатов длинноконтекстных моделей от положения значимой информации: в исследованных задачах сведения, находившиеся в середине длинного входа, нередко использовались хуже расположенных ближе к его началу или концу [48]. Работа опубликована в *Transactions of the Association for Computational Linguistics* в 2024 году. ([MIT Press Direct][1])

Этот результат не следует превращать в универсальное утверждение о современных и будущих моделях. Архитектуры, обучение и размеры контекстных окон изменяются. Для CHLOYA важен более устойчивый вывод:

> **вместимость контекстного окна и надёжность использования его содержимого являются разными свойствами.**

Anthropic в практических рекомендациях по проектированию контекста аналогично рассматривает контекст как ограниченный ресурс и формулирует задачу не как максимальное заполнение окна, а как поддержание набора сведений, наиболее полезного в текущий момент работы агента [49]. ([Антропик][2])

Поэтому увеличение технически доступного окна не отменяет необходимости проектировать отбор.

Даже при низкой стоимости обработки большого количества токенов остаются конкуренция между актуальными и историческими сведениями, противоречия, доверительные риски и сложность последующего объяснения того, на каком основании было принято решение.

### Релевантность и актуальность являются разными свойствами

Смысловой поиск способен найти документ, идеально соответствующий текущей теме, но это ещё не означает, что содержащиеся в нём сведения продолжают действовать.

Например, старая документация может подробно описывать именно ту функцию, с которой работает исполнитель, но относиться к уже заменённому интерфейсу. В таком случае высокий уровень тематического соответствия делает устаревший фрагмент даже более опасным: он выглядит непосредственно применимым к задаче.

Исследование Weng et al. 2026 года на небольшой контролируемой выборке изменений в Python-проектах показало, что устаревший репозиторный контекст способен не просто не помогать модели, а направлять её к уже неактуальному состоянию программы [53]. Авторы отдельно ограничивают выводы масштабом эксперимента, поэтому количественные значения нельзя считать универсальными, но работа подтверждает сам класс риска. ([arXiv][3])

Следовательно:

> **Найдено — не означает применимо. Релевантность — не означает актуальность.**

Для значимого знания CHLOYA должна учитывать не только предметную близость, но и время или версию применимости, область действия, происхождение и статус решения.

Это продолжает ранее сформированный в CHLOYA контур актуальности: сведения могут быть истинными в момент создания и стать ошибочными для последующей версии системы без какого-либо злонамеренного изменения.

### Актуальность нельзя свести к единому сроку жизни

Простое правило вида «через тридцать дней запись устаревает» недостаточно для проектной памяти.

Архитектурное решение может оставаться действующим годами. Описание конкретной сигнатуры функции способно потерять актуальность после ближайшего изменения кода.

Поэтому время является только одним из признаков.

Более подходящая модель связывает знание с **условиями его применимости**. Например, архитектурное решение может действовать для определённой версии компонента и требовать пересмотра при изменении конкретного контракта. Если такое событие произошло, связанные сведения получают статус требующих проверки независимо от календарного возраста.

Таким образом, актуальность проектной памяти имеет как временную, так и событийную составляющую.

> **Знание должно стареть не только по часам, но и вследствие изменений тех объектов и предпосылок, от которых оно зависит.**

Это особенно важно для автоматически формируемого контекста. Система должна по возможности знать не только дату документа, но и события, способные лишить его текущей применимости.

### Память не должна расти пропорционально активности агентов

Проектная память может начать увеличиваться значительно быстрее, чем реальное долговечное знание проекта.

Если после каждой задачи автоматически сохраняются постановка, полный ход рассуждений, промежуточные выжимки, передача работы, отчёт, краткое резюме и несколько производных представлений, через сотни задач возникает большой объём формально структурированных материалов, значительная часть которых больше не влияет на работу.

В накопленном анализе CHLOYA этот риск был выделен как **[разрастание спецификации][g-specification-bloat]**: объём формализованных описаний увеличивается быстрее, чем объём полезного знания.

Для 6.21 это становится одним из центральных антипаттернов.

> **Проектная память должна расти вместе с долговечным знанием проекта, а не вместе с числом сеансов, сообщений и действий исполнителей.**

Подробная история при этом не обязана исчезать. Git, журналы выполнения, архивы испытаний и другие средства могут сохранять её для расследования и аудита. Но сам факт существования исторического артефакта не означает необходимость поддерживать его как часть постоянно актуального канонического знания.

Особенно важно различать **событие** и **знание, извлечённое из события**. Завершённая задача является историческим событием. Архитектурное ограничение, которое было в ней обнаружено, может стать долговременным знанием. Эти две сущности не должны автоматически получать одинаковый жизненный цикл.

### Роль хранения и частота использования — разные измерения

Ранее для проектной памяти CHLOYA рассматривались как канонический, индексный и архивный слои, так и «горячая» и «холодная» память. Эти классификации описывают разные свойства и не должны смешиваться.

Канонический слой отвечает на вопрос:

> **что является сохраняемым источником проектного знания?**

Индексный слой помогает быстро найти необходимые сведения, но может быть пересоздан из канонического состояния.

Архив сохраняет исторические материалы, которые больше не должны участвовать в обычной работе как действующее состояние.

Другая ось характеризует частоту активации. Некоторые сведения нужны практически каждой задаче и могут находиться в компактном постоянно доступном ядре. Другие остаются каноническими и обязательными для проекта, но требуются только при изменении определённой подсистемы.

Наблюдательный опыт *Codified Context* [22] хорошо иллюстрирует возможность такого устройства. В описанном автором проекте примерно на 108 тысяч строк C# использовались компактное постоянно доступное ядро, специализированные агенты и 34 документа, подключаемых по необходимости; работа охватывает 283 сеанса разработки. Это опыт одного проекта, а не контролируемое доказательство универсального превосходства схемы, но он представляет практически полезный пример разделения постоянно необходимого и вызываемого по требованию знания. ([arXiv][4])

Таким образом, каноническое знание вполне может использоваться редко. Редкость использования не делает его архивным или необязательным.

### Поисковый индекс не является проектной памятью

Векторный, полнотекстовый или иной индекс может существенно ускорить доступ к проектным знаниям, но он решает более узкую задачу: помогает найти сведения, которые некоторым образом соответствуют запросу.

Он сам по себе не определяет, какое из двух противоречащих решений является действующим, какое было отменено, насколько сведения актуальны и обладают ли они нормативной силой.

Поэтому CHLOYA сохраняет различие:

> **модель постоянного проектного знания не равна индексу его поиска.**

Repoformer показал, что безусловное извлечение дополнительного репозиторного контекста не всегда полезно: значительная доля найденных фрагментов могла оказаться бесполезной или вредной, а выборочное получение контекста в исследованных авторами условиях позволило уменьшить задержку без ухудшения качества [51]. Работа опубликована на ICML 2024. ([Proceedings of Machine Learning Research][5])

Для CHLOYA это не является доказательством конкретного способа организации всей памяти. Но результат поддерживает более общий принцип:

> **сам факт возможности найти дополнительный контекст не означает, что его следует передать модели.**

Поиск должен возвращать кандидатов на включение, а не автоматически назначать им статус применимого проектного знания.

### Контекст следует расширять по выявленному дефициту знания

Другой крайностью является попытка заранее предсказать всё, что потенциально может понадобиться исполнителю, и передать это до начала работы.

Для крупного проекта такой подход снова приводит к накоплению.

CHLOYA вместо этого использует **[расширение контекста по выявленному дефициту знания][g-context-expansion-by-knowledge-gap]**.

Исполнитель начинает с минимально достаточной локальной проекции. В процессе работы он может обнаружить, что не знает действующую версию контракта, не понимает причину архитектурного решения или выявил неожиданную зависимость от соседнего модуля. Это конкретное незнание становится основанием запросить следующий слой контекста.

Критерием расширения является поэтому не количество ещё непрочитанных материалов, а наличие вопроса, существенного для корректности решения.

> **Контекст расширяется вследствие обнаруженного недостатка знания, а не ради исчерпывающего чтения всего доступного проекта.**

Такой механизм предотвращает обе крайности. Агент не получает всё состояние проекта по умолчанию, но локальная капсула также не становится жёсткой информационной клеткой: при обнаружении недостаточности она может быть расширена.

Эта идея уже присутствует в архитектуре контекста CHLOYA, где превышение первоначального бюджета допускается при обнаружении новой зависимости, повышении риска или ошибочной исходной декомпозиции.

### Сжатие не должно необратимо уменьшать проектное знание

Формирование компактного рабочего контекста неизбежно предполагает некоторую потерю деталей. Само по себе это нормально: локальному исполнителю не требуется вся история обсуждения каждого решения.

Проблема возникает, если сокращённое представление начинает заменять более полный источник.

Например, большое архитектурное решение можно сократить до фразы:

> «использовать очередь сообщений».

Такая запись способна сохранить принятое действие, но потерять причину отказа от синхронного взаимодействия, ограничения выбранного решения и условия его последующего пересмотра.

В накопленном анализе CHLOYA этот класс потерь обозначен как **[регрессия знания][g-knowledge-regression]**: новое представление выглядит более аккуратным или компактным, но фактически содержит меньше значимого знания, чем предыдущее.

Отсюда следует правило:

> **Сжатие рабочего контекста может быть потерным. Сжатие канонического проектного знания не должно происходить незаметно и необратимо.**

Краткая проекция должна по возможности сохранять ссылку на более полный источник, чтобы при возникновении противоречия или дефицита исполнитель мог восстановить исходное основание.

Тем самым проектная память сохраняет происхождение не только внешних сведений, но и собственных производных представлений.

### Богатая память и компактная рабочая проекция совместимы

PROJECTMEM [23] предлагает близкую архитектурную идею: история разработки сохраняется в виде добавляемого журнала типизированных событий, из которого детерминированно формируются компактные представления для ИИ-исполнителя. Авторы рассматривают собственную реализацию в течение двух месяцев на десяти проектах и 207 зарегистрированных событиях. Это наблюдательная самооценка, поэтому её количественные результаты следует использовать осторожно, но сама архитектура показывает, что богатое сохранённое состояние и компактный рабочий контекст не являются взаимоисключающими целями. ([arXiv][6])

CHLOYA добавляет к этому требование разделять канонический источник и производное представление. Удобная автоматически созданная выжимка не должна становиться единственным носителем причины решения, если исходное знание имело более богатую структуру.

Это особенно важно при многократном пересказе: последовательность автоматических сокращений способна постепенно изменить смысл даже без явной ошибки на одном конкретном шаге.

### Жизненный цикл проектного знания

Проектная память не должна быть пассивным складом файлов. Для её устойчивости нужен жизненный цикл, в котором новое знание проходит от возникновения к проверяемому каноническому состоянию, а в активный контекст попадает уже как задача-зависимая проекция.

```mermaid
flowchart TB
    A["Событие, наблюдение или новый результат"]
    B["Проверка значимости, происхождения и применимости"]
    C["Каноническая проектная память"]
    D["Производные индексы и связи"]
    E["Отбор под текущую задачу"]
    F["Контекстная проекция"]
    G["Активный рабочий контекст"]
    H["Результат работы"]
    I["Новое долговечное знание"]
    J["Историческое или заменённое состояние"]
    K["Архив"]
...
**Рисунок 6.21.1 — Жизненный цикл проектного знания и формирование рабочего контекста.**

Принципиально важно, что результат каждого агентного шага не возвращается в каноническую память автоматически. Сначала должно быть определено, возникло ли новое долговечное знание вообще и какой статус оно имеет.

Иначе возникает самоподдерживающийся цикл:

> агент создаёт текст → текст автоматически становится памятью → память попадает следующему агенту → следующий агент создаёт пересказ памяти → пересказ снова повышается до канона.

Такой процесс способен увеличивать объём документации без соответствующего роста знания и постепенно размывать происхождение утверждений.

### Историческое знание не следует выдавать за текущее

Потерявшее актуальность решение не всегда следует удалять физически. Оно может быть необходимо для аудита, расследования или понимания эволюции архитектуры.

Однако оно не должно продолжать участвовать в рабочем контексте на тех же основаниях, что и действующее решение.

Поэтому память должна уметь различать как минимум действующее знание, предварительное знание, заменённое состояние и архивную историю. Особенно полезна явная связь:

> **«решение A заменено решением B»**,

а не простое перемещение старого файла в архив.

Тогда поиск может вернуть старое решение как исторически релевантное, но система понимает, что оно не является текущим основанием для действия.

Это также помогает человеку: проектная память перестаёт быть набором документов, среди которых необходимо вручную угадывать наиболее новый.

### Контекстный бюджет является сигналом, а не жёстким лимитом

CHLOYA уже рассматривает контекстный бюджет как механизм управления, а не как абсолютный запрет на превышение определённого количества токенов.

Это особенно важно для 6.21.

Фиксированное правило вида «любая задача должна помещаться в N токенов» быстро устареет вместе с моделями и не учитывает сложность самой задачи.

Гораздо полезнее использовать рост рабочего контекста как диагностический сигнал.

Если локальное изменение, которое раньше требовало небольшой области знания, внезапно начинает систематически требовать чтения значительной части всей памяти проекта, возможны по меньшей мере две причины: задача перестала быть локальной либо архитектура и память больше не позволяют локализовать знание.

Таким образом, контекстный бюджет помогает выявлять не только стоимость ИИ, но и деградацию модульности.

> **Рост требуемого контекста может быть симптомом роста скрытой связанности проекта.**

Это делает управление контекстом частью архитектурной диагностики.

### Общая память и рабочий контекст должны расти с разной скоростью

Из принципа локальной достаточности следует проверяемое ожидание.

Если общая проектная память увеличилась в десять раз, активный контекст типичной локальной задачи не должен автоматически увеличиваться в те же десять раз.

Иначе локализация знания фактически не работает.

Полезно поэтому наблюдать не только абсолютный объём контекста, но и его динамику при росте проекта. Для устойчивой модульной системы ожидается, что объём памяти способен значительно увеличиваться, тогда как медианный рабочий контекст локальных задач растёт существенно медленнее.

Это не следует превращать в универсальное числовое соотношение: разные классы проектов имеют различную связанность. Но сама зависимость является хорошим объектом экспериментальной проверки.

### Полезность фрагмента нельзя надёжно определить по факту его упоминания моделью

Старая версия риска предлагала измерять долю реально использованных частей переданного контекста. Такой показатель трудно определить однозначно.

Модель может соблюдать ограничение, не упоминая его в ответе. Она может процитировать документ, который фактически не повлиял на результат. Поэтому прямой подсчёт «использованных фрагментов» способен давать ложное ощущение точности.

Более надёжный способ — проверять влияние контекста экспериментально.

Если удалить определённое решение из [контекстной проекции][g-context-projection], изменяется ли качество результата? Если вместо текущего контракта передать устаревший, появляется ли несовместимость? Если скрыть определённый инвариант, растёт ли число его нарушений?

Такой подход позволяет оценивать не словесное внимание модели, а **фактический вклад элемента контекста в результат**.

Он также согласуется с проверкой с исключением компонентов, уже введённой в 6.17.

### Человек также является потребителем проектной памяти

Перегрузка памяти является проблемой не только модели.

Если после нескольких лет работы человек не способен определить, какое решение является действующим, почему оно было принято и где находится первичный источник, проект потерял одно из базовых свойств CHLOYA независимо от того, насколько хорошо работает автоматический поиск.

Поэтому оптимизация памяти только под ИИ недостаточна.

Каноническое состояние должно оставаться человекочитаемым, пригодным для сравнения версий и восстановления хода значимых решений. Это соответствует уже принятому принципу CHLOYA: проектная память принадлежит проекту, а индексы, базы и другие ускоряющие представления являются производными слоями.

> **Если состояние проекта понятно только системе, которая его индексирует, переносимая проектная память фактически утрачена.**

### Исследовательская гипотеза

**H6.21.** Разделение полной проектной памяти и активного рабочего контекста, формирование задача-зависимых контекстных проекций, учёт актуальности и области применимости сведений и [расширение контекста по выявленному дефициту знания][g-context-expansion-by-knowledge-gap] позволяют уменьшить объём нерелевантного и устаревшего материала без практически значимого ухудшения качества выполнения задач, переносимости между исполнителями и способности восстанавливать причины решений.

Дополнительная гипотеза:

**H6.21a.** При росте проектной памяти объём активного контекста типичной локальной задачи может расти существенно медленнее общего объёма сохранённого знания без практически значимого ухудшения качества результата.

Эти гипотезы не предполагают, что минимальный контекст всегда является лучшим. Слишком агрессивное сокращение может привести к пропуску системного ограничения или потере причины решения. Исследуется именно **минимальная достаточность**, а не минимальный размер.

### Экспериментальная проверка

Проверку следует проводить на развивающемся проекте, а не только на статическом наборе файлов. В ходе эксперимента должны появляться новые решения, изменяться контракты, отменяться старые ограничения и сохраняться исторические документы. Только в таком режиме можно проверить способность памяти различать тематическую релевантность и текущую применимость.

Первый режим предоставляет исполнителю максимально доступный объём памяти в пределах технического окна модели. Второй использует обычный поиск по проектной памяти без специальной обработки актуальности. Третий применяет контур CHLOYA: компактное постоянное ядро, локальную проекцию задачи, статус и область применимости сведений и расширение контекста только после обнаружения конкретного дефицита знания. Четвёртый режим намеренно использует чрезмерно сокращённую проекцию, чтобы измерить противоположный риск — потерю необходимого проектного знания.

Проверка должна повторяться по мере роста истории проекта. Это позволит определить, сохраняется ли преимущество локализации, когда объём памяти увеличивается в несколько раз, и не начинает ли стоимость её сопровождения превышать получаемую пользу.

### Показатели оценки

| Область                | Основные показатели                                                                                                          |
| ---------------------- | ---------------------------------------------------------------------------------------------------------------------------- |
| Рабочий контекст       | Объём переданной информации, стоимость её обработки, динамика размера типичной локальной задачи при росте всей памяти        |
| Достаточность          | Число пропусков критически необходимого знания, обоснованных расширений контекста и остановок из-за недостаточности сведений |
| Актуальность           | Ошибки, вызванные заменёнными или устаревшими сведениями; доля найденных материалов, правильно распознанных как исторические |
| Сжатие                 | Потеря ограничений, причин и условий применимости при формировании кратких представлений                                     |
| Практический результат | Качество принятого изменения, повторная работа, способность другого исполнителя и человека восстановить решение              |
| Стоимость памяти       | Человеческие и вычислительные затраты на обновление, проверку, поиск, архивирование и поддержание согласованности            |

Отдельно полезно наблюдать случаи, когда контекстный бюджет систематически превышается. Они могут указывать не на недостаточный размер бюджета, а на рост связанности архитектуры или неудачное устройство самой памяти.

### Объект и предмет исследования

**Объект исследования** — проектная память и формируемый из неё рабочий контекст в длительной программной разработке с ИИ-исполнителями.

**Предмет исследования** — влияние локализации, актуальности, контекстных проекций, управляемого расширения и жизненного цикла знания на размер рабочего контекста, качество результата, переносимость и стоимость сопровождения проекта.

### Угрозы валидности

Первая угроза связана с развитием длинноконтекстных моделей. Ограничения, наблюдавшиеся в более ранних моделях [48], могут со временем ослабевать, поэтому преимущество локализации нельзя обосновывать только техническим размером контекстного окна. Более устойчивыми основаниями остаются актуальность, применимость, происхождение и стоимость проверки.

Вторая угроза состоит в том, что автоматический механизм отбора может скрыть необходимое знание. Высокая точность поиска не гарантирует полноту сведений, необходимых для конкретного изменения. Поэтому исполнитель должен иметь возможность запросить расширение и явно зафиксировать обнаруженный дефицит.

Третья угроза возникает при оценке актуальности. Система способна ошибочно пометить действующее решение как заменённое или, наоборот, продолжить считать устаревшее ограничение текущим. Поэтому автоматическое распространение статусов должно сохранять проверяемые основания и возможность человеческого исправления.

Четвёртая угроза — постепенная потеря знания при многократном сжатии. Каждая отдельная выжимка может выглядеть приемлемой, но несколько поколений производных резюме способны удалить существенные причины и исключения. Для значимых решений поэтому должен сохраняться доступ к более первичному каноническому источнику.

Пятая угроза связана с стоимостью самой инфраструктуры памяти. Сложная система классификации, индексации, актуализации и связей способна потребовать больше ресурсов, чем экономит. Это непосредственно возвращает 6.21 к мета-риску 6.17: механизм памяти полезен только тогда, когда его совокупная практическая ценность превышает собственные накладные расходы.

Наконец, результаты пилота зависят от структуры проекта. Хорошо модульная система естественным образом облегчает локализацию контекста, тогда как сильно связанный проект может действительно требовать большой области знания. Методология не должна искусственно скрывать системную связанность ради красивой метрики маленького контекста.

### Граница применимости

CHLOYA не требует минимизировать проектную память.

Долгоживущий проект может и должен накапливать значительный объём полезного знания. Цель состоит в том, чтобы увеличение этого объёма не приводило автоматически к увеличению информации, которую обязан одновременно обрабатывать каждый исполнитель.

Не требуется также всегда начинать с малого контекста. Для системной архитектурной задачи широкое представление проекта может быть необходимо с самого начала. Минимальная достаточность определяется содержанием задачи, а не желаемым числом токенов.

Поиск, автоматическое суммирование и индексы допустимы и полезны, если остаются производными механизмами доступа к переносимому состоянию проекта. Они не должны незаметно заменять каноническое знание или лишать человека возможности определить, какое решение является действующим.

Основной принцип раздела:

> **Проектная память является источником рабочего контекста, а не самим рабочим контекстом.**

Второй принцип:

> **Сжатие рабочего контекста не должно становиться необратимым сжатием знания проекта.**

Третий:

> **Релевантность найденного сведения не доказывает его актуальность и применимость.**

И итоговая формула раздела:

> **Хорошая проектная память не заставляет исполнителя помнить всё. Она позволяет получить именно то знание, которое необходимо сейчас, и восстановить более полное основание тогда, когда задача этого потребует.**
...
Анализ рисков показывает, что устойчивость агентной разработки нельзя свести к качеству отдельной модели. Ошибка может возникнуть у исполнителя, человека, координатора, во внешнем источнике, в проектной памяти или в самом механизме управления. Поэтому CHLOYA не предполагает существования безошибочного участника. Её задача — ограничивать полномочия и область последствий, сохранять основания решений, обеспечивать проверяемость переходов и позволять ошибке быть обнаруженной до того, как она распространится на значительную часть проекта.

Из этого следует несколько общих принципов.

**Локальность не должна уничтожать системный смысл.** Исполнитель получает минимально достаточную область работы, но существенные связи с исходной целью, решениями и инвариантами должны оставаться восстанавливаемыми.

**Контекст, доверие и полномочие являются разными сущностями.** Полученная информация способна изменить понимание задачи, но не должна сама создавать новые права на действие. Пересказ не повышает доверие автоматически, техническая доступность инструмента не означает разрешения его использовать, а новый внешний артефакт не наследует доверие предыдущей версии без отдельного основания.

**Ценность проверки определяется не её количеством, а тем, какую неопределённость она действительно уменьшает.** Несколько исполнителей, использующих одну ошибочную предпосылку, не создают нескольких независимых подтверждений. Аналогично хеш, фиксация версии или происхождение артефакта подтверждают отдельные свойства, но не являются полной гарантией безопасности.

**Риск определяется не только вероятностью ошибки, но и радиусом её распространения.** Решение, от которого зависит множество последующих работ, требует более ранней и сильной проверки, чем локальная обратимая операция. Высокая скорость параллельной генерации полезна лишь до тех пор, пока проверка и интеграция способны удерживать объём непроверенной работы.

**Больше механизмов не означает лучше.** Контракты, проверки, роли, документы, ограничения и проектная память сами создают стоимость. Их сложность оправданна только наблюдаемой функцией и должна быть пропорциональна риску.

Последний принцип относится к самой CHLOYA. Методология не должна быть защищена от собственного отрицательного результата. Если механизм не даёт подтверждаемой пользы в предполагаемой области применения, его необходимо пересматривать. Если облегчённый профиль устойчиво эффективнее расширенного, предпочтение должно получать облегчённое решение. Если после упрощения и нескольких реалистичных проверок CHLOYA увеличивает совокупную стоимость процесса без практически значимого выигрыша в качестве, управляемости, переносимости или снижении риска, под вопрос должна ставиться уже сама методология.

> **Практическая ценность CHLOYA определяется не полнотой её спецификации, а способностью решать наблюдаемые проблемы разработки с приемлемой полной стоимостью и проверяемыми последствиями.**

Поэтому результатом главы 6 является не утверждение, что перечисленные риски уже устранены. Глава формирует **контур проверяемости CHLOYA**: что именно предполагается улучшить, какие побочные эффекты необходимо учитывать и какие результаты должны привести к изменению исходных механизмов.

Следующий этап развития методологии должен переводить эти положения в пилоты и сравнительные проверки, связывая гипотезы с показателями, контрольными режимами, практически значимыми порогами и условиями пересмотра. Только после такой проверки отдельные положения CHLOYA могут переходить от обоснованных гипотез к эмпирически подтверждённым рекомендациям.

[g-external-artifact-promotion]: ../../research/GLOSSARY.ru.md#продвижение-внешнего-артефакта
[g-protective-delay]: ../../research/GLOSSARY.ru.md#защитная-задержка
[g-observation-period]: ../../research/GLOSSARY.ru.md#период-наблюдения
[g-urgent-dependency-adoption]: ../../research/GLOSSARY.ru.md#срочное-принятие-зависимости
[g-external-artifact-identity]: ../../research/GLOSSARY.ru.md#идентичность-внешнего-артефакта
[g-artifact-trust-scope]: ../../research/GLOSSARY.ru.md#область-доверия-артефакта
[g-promotion-boundary]: ../../research/GLOSSARY.ru.md#граница-продвижения
[g-fresh-release-risk]: ../../research/GLOSSARY.ru.md#риск-свежего-выпуска
[g-outdated-version-risk]: ../../research/GLOSSARY.ru.md#риск-устаревшей-версии
[g-error-cascading]: ../../research/GLOSSARY.ru.md#каскадирование-ошибки
[g-pre-branching-verification]: ../../research/GLOSSARY.ru.md#проверка-перед-разветвлением
[g-error-propagation-radius]: ../../research/GLOSSARY.ru.md#радиус-распространения-ошибки
[g-rework-multiplier]: ../../research/GLOSSARY.ru.md#множитель-повторной-работы
[g-unverified-work-budget]: ../../research/GLOSSARY.ru.md#бюджет-непроверенной-работы
[g-reverse-error-escalation]: ../../research/GLOSSARY.ru.md#обратная-эскалация-ошибки
[g-context-projection]: ../../research/GLOSSARY.ru.md#контекстная-проекция
[g-context-expansion-by-knowledge-gap]: ../../research/GLOSSARY.ru.md#расширение-контекста-по-выявленному-дефициту-знания
[g-specification-bloat]: ../../research/GLOSSARY.ru.md#разрастание-спецификации
[g-knowledge-regression]: ../../research/GLOSSARY.ru.md#регрессия-знания
[g-validity-conditions]: ../../research/GLOSSARY.ru.md#условия-актуальности
[g-project-knowledge-state]: ../../research/GLOSSARY.ru.md#состояние-проектного-знания
